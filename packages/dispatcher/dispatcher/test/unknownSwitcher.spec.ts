...
describe("selectFromPartitions", () => {
    ...
    test("all partitions run in parallel", async () => {
        ...
        // Parallel: total time should be well under sequential (partitionCount × delayMs).
        // This assertion compares measured elapsed time against sequential baseline
        // to reduce CI flakiness from absolute timing jitter.
        const partitionCount = partitions.length;
        const sequentialMs = delayMs * partitionCount;
        expect(elapsed).toBeLessThan(sequentialMs * 0.7);
    });

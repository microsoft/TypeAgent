...
def mock_azure_token_provider(mocker: MockerFixture) -> AzureTokenProvider:
    ...
    mock_credential = mocker.patch("typeagent.aitools.auth.DefaultAzureCredential").return_value
    provider = AzureTokenProvider()
    provider.credential = mock_credential
    return provider


def test_get_token_with_refresh(mock_azure_token_provider: AzureTokenProvider, mocker: MockerFixture):
    ...
    mock_refresh_token = mocker.patch.object(mock_azure_token_provider, "refresh_token", return_value="new_token")
...
def test_get_token_without_refresh(mock_azure_token_provider: AzureTokenProvider, mocker: MockerFixture):
...
def test_refresh_token(mock_azure_token_provider: AzureTokenProvider, mocker: MockerFixture):
    """Test that refresh_token retrieves a new token."""
    mock_get_token = mocker.patch.object(
        mock_azure_token_provider.credential,
        "get_token",
        return_value=mocker.MagicMock(token="new_token", expires_on=int(time.time()) + 3600),
    )

    token = mock_azure_token_provider.refresh_token()

    mock_get_token.assert_called_once_with("https://cognitiveservices.azure.com/.default")
    assert token == "new_token"
...
def test_needs_refresh_expired_token(mock_azure_token_provider: AzureTokenProvider, mocker: MockerFixture):
...
def test_needs_refresh_valid_token(mock_azure_token_provider: AzureTokenProvider, mocker: MockerFixture):

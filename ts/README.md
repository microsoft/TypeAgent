...
The follow set of functionality will need the services keys. Please read the links for details about the variables needed. It is possible to use "keyless" configuration for some APIs.  See [Running Keyless](#keyless) below.
...
**Running Keyless**
It is possible to run a subset of the TypeAgent endpoints in a keyless environment for additional security. Instead of using keys, the examples provided use Azure Entra user identities to authenticate against endpoints. To use this approach, modify the .env file and specify **identity** as the key value.
You must also configure your services to assign the correct [RBAC](https://learn.microsoft.com/en-us/azure/role-based-access-control/overview) to users access to endpoints. 
...
options for elevation (duration, justification message, etc.) are stored in `tools/scripts/elevate.config.json`.

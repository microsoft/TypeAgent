...
export type CalendarDateTime = {
    ...
    // examples: "-1y" for previous year, "+1y" for next year, "-2y" for two years ago; never used unless the user specifies a relative date or an absolute year by giving the year as a number in the request
    year?: string;
    // examples: "July", "-1M" for last month, "+3M" for three months from now; default is this month
    month?: string;
    // examples: "-1w" for last week; default is this week
    week?: string;
    // examples: "10", "-1d", "+3d", "4", "Monday", "StartOfWeek", "EndOfMonth"
    // non-examples (never use): "StartOfDay", "EndOfDay" (just leave undefined to indicate "today")

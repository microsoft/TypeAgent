...
export default class Filter extends Command {
  ...
  static args = {
    files: Args.string({
      description: 'Path(s) to input JSON file(s) containing restaurant data',
      required: true,
      multiple: true,
    }),
  };
  ...
  private normalizeRestaurant(restaurant: FilterRestaurant): FilterRestaurant {
    if (
  ...
  private filterTripAdvisor(restaurants: FilterRestaurant[]): FilterRestaurant[] {
    return restaurants
  ...
  async run(): Promise<void> {
    ...
    const inputFilePaths: string[] = Array.isArray(args.files) ? args.files : [args.files];

    if (inputFilePaths.length === 0) {
      this.error('❌ Please provide one or more JSON file paths as arguments.');
      return;
    }

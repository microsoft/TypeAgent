...
export default class Merge extends Command {
    ...
    private mergeRestaurants(parsed: BaseRestaurant, crawl: BaseRestaurant): BaseRestaurant {
      return {
        ...parsed,
    ...
    async run(): Promise<void> {
      ...
      if (!Array.isArray(parsedData) || !Array.isArray(crawlData)) {
        this.error('Expected JSON arrays of restaurant objects in both input files.');
        return;
      }
      ...
      crawlData.forEach((crawlEntry) => {
        ...
        if (match) {
          ...
          mergedOnlyOverlap.push(this.mergeRestaurants(match, crawlEntry));
        } else {

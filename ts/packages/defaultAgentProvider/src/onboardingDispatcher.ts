...
const debug = registerDebug("typeagent:studio:onboarding:dispatcher");

/**
 * Populate `process.env` with the repo's service keys (OpenAI / Azure endpoints
 * + keys) once, before the dispatcher builds any LLM client.
 *
 * The Studio service process — where this dispatcher runs (loaded via a lazy
 * external import) — does not otherwise load any keys, so Discovery's LLM
 * translation would fail with an auth error. `loadConfigSync` mirrors how the
 * rest of TypeAgent resolves keys: it auto-detects the `ts/` workspace root and
 * merges `config.defaults.yaml` + `config.local.yaml` + the legacy `.env`,
 * populating only vars that are still undefined (so a process already launched
 * with keys in its environment is never clobbered). Best-effort and idempotent:
 * a missing or malformed config never blocks the wizard here — a genuinely
 * missing key surfaces later as a clear translation/auth error.
 */
let keysLoaded = false;
function ensureServiceKeysLoaded(): void {
    if (keysLoaded) {
        return;
    }
    keysLoaded = true;

...
The `code-agent` is a TypeAgent application agent designed to automate tasks in Visual Studio Code (VSCode). It acts as a dispatcher for code-related actions, enabling users to interact with VSCode through natural language commands. This agent is tightly integrated with the [coda](../../coda/README.md) VSCode extension, which must be deployed for the `code-agent` to function.

## What it does

The `code-agent` currently implements the `launchVSCode` action, which allows users to start or open VSCode in different modes:
...
The `code-agent` package is structured around several key files:

- **[codeManifest.json](./src/codeManifest.json)**: This manifest file defines the agent's metadata, including its description, schema, and sub-agents. It is the primary entry point for understanding the agent's configuration.
- **[codeActionsSchema.ts](./src/codeActionsSchema.ts)**: Contains the schema for all actions supported by the agent, including their names and parameters. For example, the `launchVSCode` action is defined here.
- **[codeSchema.agr](./src/codeSchema.agr)**: Defines the natural language grammar for mapping user commands to actions. This file enables the agent to interpret user input and match it to the appropriate action.
- **[codeActionHandler.ts](./src/codeActionHandler.ts)**: Implements the logic for handling actions. For instance, the `launchVSCode` action is processed in this file.
...
To extend the `code-agent` with new functionality, follow these steps:
...
By following these steps, you can expand the `code-agent` to support additional VSCode automation tasks.
...
_Auto-generated against commit `44b34a9ac8794b6f90489ff7e55fe57283c34960` on `2026-07-11T08:34:41.338Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter code-agent docs:verify-links` to spot-check._

...
The `music-local` agent enables users to interact with their local music library through natural language commands. It supports the following key functionalities:
...
- **File Search and Listing**: Actions like `listFiles` and `searchFiles` enable users to browse and locate audio files in their music folder.
...
The `music-local` package is structured around several key files that define its functionality:
...
To extend the `music-local` agent, you can add new actions, modify existing ones, or enhance its capabilities. Here’s how:
...
_Auto-generated against commit `656444843518fd1f9bb1b157b6dbf6dcbcde3999` on `2026-07-09T09:05:44.186Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter music-local docs:verify-links` to spot-check._

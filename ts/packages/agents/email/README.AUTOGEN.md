...
The Email agent provides the following key actions for managing email:

- **`sendEmail`**: Allows users to send an email with a subject, body, recipients, and optional attachments. It supports CC, BCC, and attachments specified as file paths or URLs.
...
- **`findEmail`**: Searches for an email message using a `MessageReference`, which can include details like the subject or content of the email.

These actions are implemented using the Microsoft Graph API, which provides access to email accounts, messages, and related operations. The agent also integrates with the `graph-utils` library for email-specific functionality and supports generating email content using LLMs.
...
1. **Create a Microsoft Graph Client Application**:
   ...
   - Visit the Microsoft Graph quickstart page to create a Graph Client application (`https://developer.microsoft.com/en-us/graph/quick-start?state=option-typescript`).
...
- **[emailSchema.agr](./src/emailSchema.agr)**: Defines the grammar for parsing user requests into actionable commands. This file ensures that user inputs are correctly interpreted and mapped to the appropriate actions.
...
_Auto-generated against commit `44b34a9ac8794b6f90489ff7e55fe57283c34960` on `2026-07-12T08:45:00.858Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter email docs:verify-links` to spot-check._

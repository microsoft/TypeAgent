...
const COLLISION_TERMS = [
    "play", "start", "stop", "pause", "resume",
    "search", "find", "list", "show", "display",
    "open", "close", "navigate", "go to",
    "click", "scroll", "select", "type",
    "read", "write", "save", "load"
];

function extractPatternsFromAGR(filePath: string, namespace: string): PatternInfo[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentAction) {
            ...
            if (nextLine.startsWith("->") && !line.includes("->")) {
                ...
                if (pattern && !pattern.startsWith("//") && !pattern.startsWith("import")) {
                    patterns.push({
                        namespace,
...
function analyzePattern(pattern: PatternInfo): { severity: "high" | "medium" | "low"; issue: string; suggestion?: string }[] {
    const issues: { severity: "high" | "medium" | "low"; issue: string; suggestion?: string }[] = [];
    ...
    const poeticTerms = ["summon", "conjure", "behold", "peruse", "traverse", "manifest"];
    for (const term of poeticTerms) {
        if (p.includes(term)) {
            issues.push({
                severity: "medium",
                issue: `Contains poetic/literary term: "${term}"`,
                suggestion: "Use common vernacular (e.g., 'show', 'get', 'list')"
            });
        }
    }
    ...
    if (pattern.namespace === "files" && pattern.action === "listFiles") {
        const hasCommonVerb = ["list", "show", "display", "ls", "dir"].some(v => p.includes(v));
        if (!hasCommonVerb) {
            issues.push({
                severity: "medium",
...
function checkCollisionRisks(patterns: PatternInfo[]): { term: string; patterns: PatternInfo[] }[] {
...
async function auditNamespace(namespace: string, detailed: boolean): Promise<AuditResult> {
    const schemaPath = namespace === "powershell"
...
async function main() {
    ...
    const namespaces = namespaceFilter
        ? [namespaceFilter]
        : ["files", "processes", "services", "system", "network", "data", "archives"];
        ...
        try {
            ...
            if (detailed && result.issues.length > 0) {
                ...
                for (const issue of result.issues) {
                    console.log(`    [${issue.severity.toUpperCase()}] Line ${issue.pattern.lineNumber}: ${issue.pattern.pattern}`);
    ...
    // Overall statistics
    const totalPatterns = allResults.reduce((sum, r) => sum + r.patterns.length, 0);
    const totalIssues = allResults.reduce((sum, r) => sum + r.issues.length, 0);
    const highSeverity = allResults.reduce((sum, r) => sum + r.issues.filter(i => i.severity === "high").length, 0);
    const mediumSeverity = allResults.reduce((sum, r) => sum + r.issues.filter(i => i.severity === "medium").length, 0);
    const lowSeverity = allResults.reduce((sum, r) => sum + r.issues.filter(i => i.severity === "low").length, 0);

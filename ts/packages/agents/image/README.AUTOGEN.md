...
The `image-agent` package is an image dispatcher agent that processes user requests for creating and editing images. It leverages Azure OpenAI's `gpt-image-1` endpoints to generate and transform images based on user-provided descriptions and prompts. This agent is part of the TypeAgent monorepo and resides in the `ts/packages/agents/image/` directory.

## What it does

The `image-agent` provides two primary actions for handling image-related tasks:
...
- **`editImageAction`**: This action allows users to modify or transform an existing image. The user provides an edit prompt (e.g., "make this a watercolor painting") and the source image to be edited. The agent processes the request and applies the specified transformation using the Azure OpenAI API. This is useful for tasks like stylizing images or applying artistic effects.

These actions enable the agent to handle a variety of use cases, from generating new images to creatively transforming existing ones.
...
- **[src/imageManifest.json](./src/imageManifest.json)**: Contains metadata about the agent, including its description, emoji representation, and schema details.
...
- **[src/imageActionHandler.ts](./src/imageActionHandler.ts)**: Implements the logic for handling the defined actions. This file processes user requests and interacts with the Azure OpenAI API to generate or edit images.
- **[src/imageSchema.agr](./src/imageSchema.agr)**: Contains grammar rules for parsing user requests into actionable commands. This ensures that natural language inputs are correctly interpreted.
...
By following these steps and adhering to the existing patterns in the codebase, you can expand the `image-agent` to support additional image-related actions or integrate with new APIs.
...
_Auto-generated against commit `656444843518fd1f9bb1b157b6dbf6dcbcde3999` on `2026-07-09T09:05:44.186Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter image-agent docs:verify-links` to spot-check._

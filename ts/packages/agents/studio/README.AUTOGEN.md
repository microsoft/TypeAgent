...
The `studio-agent` package is a TypeAgent application agent that serves as the conversational interface for the TypeAgent Studio runtime. It enables developers to inspect, monitor, and debug the Studio environment by providing read-only access to key information such as the repository structure, schema collisions, and event logs. This agent is an integral part of the TypeAgent ecosystem, supporting the development and validation of other agents within the Studio framework.

## What it does

The `studio-agent` provides three primary actions, all of which are read-only and designed to facilitate the inspection and monitoring of the TypeAgent Studio environment:

- **`getStudioInfo`**: This action reports the current Studio environment, including the repository root and the directories ("agent locations") being scanned for agents. It also provides the count of agent packages in each directory. This is particularly useful for verifying that Studio is correctly configured and scanning the intended locations.
...
These actions are designed to provide developers with insights into the Studio environment, helping them identify and resolve issues, monitor recent activities, and ensure the system is functioning as expected.
...
_Auto-generated against commit `44b34a9ac8794b6f90489ff7e55fe57283c34960` on `2026-07-12T08:45:00.858Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter studio-agent docs:verify-links` to spot-check._

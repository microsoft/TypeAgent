...
The `taskflow-typeagent` package is a TypeAgent application agent that enables users to create, manage, and execute task flows. These task flows act as user-defined macros, allowing for the automation of sequences of TypeAgent actions. By defining reusable workflows, this package simplifies the execution of complex tasks within the TypeAgent ecosystem.
...
- `deleteTaskFlow`: Removes a specific task flow by name, enabling users to clean up or update their task flow library.

The package also includes features for defining task flows using natural language grammar patterns, which are generated programmatically and can be customized by users. It provides a script execution environment for running task flows programmatically, as well as tools for validating and managing task flow scripts.
...
- [actionHandler.mts](./src/actionHandler.mts): Implements the logic for executing task flow actions, managing the task flow store, and handling user-defined macros.
...
- [grammarGenerator.ts](./src/grammarGenerator.ts): Contains logic for generating natural language grammar patterns for task flow actions. It supports optional integration with language models (LLMs) to assist in generating patterns.
...
By following these steps, you can expand the capabilities of the `taskflow-typeagent` package to support additional workflows and use cases.
...
_Auto-generated against commit `44b34a9ac8794b6f90489ff7e55fe57283c34960` on `2026-07-12T08:45:00.858Z` by `docs-generate.yml`. Links validated at that commit; the working tree may have drifted by up to 24h. Re-run `pnpm --filter taskflow-typeagent docs:verify-links` to spot-check._

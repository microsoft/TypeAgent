...
function createBuildOptions(outDir, options = {}) {
    const providedRollup = options.rollupOptions || {};
    const providedExternal = providedRollup.external;
    // jsdom is only used by the Node-side createNodeHtmlReducer() path in
    // @typeagent/browser-control-rpc, which the extension never invokes.
    // Keep it external so it is not bundled into the browser/electron output.
    const external = Array.isArray(providedExternal)
        ? [...new Set([...providedExternal, "jsdom"])]
        : providedExternal
          ? providedExternal
          : ["jsdom"];
    return {
        ...
        build: {
            ...
            sourcemap: isDev,
            minify: !isDev,
            rollupOptions: { ...providedRollup, external },
        },
    };
}
...
const sharedScripts = {
    ...
    "views/memoryCenter": "views/memoryCenter.ts",

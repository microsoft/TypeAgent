...
const IMPORTANCE_LEVELS: ImportanceLevel[] = [
    { level: 1, threshold: 0.8, maxNodes: 1000, description: "Critical Nodes Only" },
    { level: 2, threshold: 0.5, maxNodes: 5000, description: "Important Nodes" },
...
export async function getImportanceStatistics(
    ...
    try {
        ...
        const entitiesWithMetrics = calculateEntityMetrics(entities, relationships, communities);

        // Calculate importance distribution
        const importanceScores = entitiesWithMetrics.map(e => e.importance || 0).sort((a, b) => b - a);

        // Preview node counts at each level
        const levelPreviews = IMPORTANCE_LEVELS.map(level => ({
            level: level.level,
            nodeCount: importanceScores.filter(score => score >= level.threshold).length,
            coverage: importanceScores.filter(score => score >= level.threshold).length / importanceScores.length
        }));

        // Recommend level based on graph size
        const totalNodes = entities.length;
        const recommendedLevel = totalNodes > 25000 ? 1 : totalNodes > 10000 ? 2 : totalNodes > 3000 ? 3 : 4;
...
function ensureGlobalConnectivity(
    ...
    const components = findConnectedComponents(importantEntities, allRelationships);

    // If multiple components, add bridge nodes to connect them
    if (components.length > 1) {
        const bridgeNodes = findBridgeNodes(components, allRelationships, maxNodes - importantEntities.length);
...
function findConnectedComponents(entities: any[], relationships: any[]): any[][] {
...
function findBridgeNodes(components: any[][], allRelationships: any[], maxBridgeNodes: number): any[] {
...
function analyzeConnectivity(entities: any[], relationships: any[]): any {
    ...
    return {
        ...
        averageComponentSize: components.reduce((sum, c) => sum + c.length, 0) / components.length
    };
}

function buildImportanceWeightedAdjacency(entities: any[], relationships: any[]): Map<string, Array<{entity: any, importance: number}>> {
    const adjacencyMap = new Map<string, Array<{entity: any, importance: number}>>();
    const entityMap = new Map<string, any>();

...
async function extractKnowledgeFromPage(
    ...
    console.log(`Extracting knowledge from: ${parameters.title} (${parameters.url})`);
    ...
    try {
        ...
        if (response.success && response.data) {
            console.log(`Knowledge extraction completed: ${response.data.entities.length} entities, ${response.data.relationships.length} relationships`);
            return response.data;
        } else {
            console.error("Knowledge extraction failed:", response.success ? "No data returned" : "Request failed");
            return {
                entities: [],
                relationships: [],
                keyTopics: [],
                suggestedQuestions: [],
                summary: "Knowledge extraction failed: " + (response.success ? "No data returned" : "Request failed"),
            };
        }
...
): Promise<{ indexed: boolean; knowledgeExtracted: boolean; entityCount: number }> {
    ...
    try {
        ...
        if (parameters.extractKnowledge && textContent.length > 100) {
            try {
                ...
                console.error('Knowledge extraction failed during indexing:', error);
            }
        }
        
        console.log(`Page indexed successfully: ${entityCount} entities extracted`);
...
async function queryWebKnowledge(
    ...
    console.log(`Querying web knowledge: "${parameters.query}" (scope: ${parameters.searchScope})`);
    ...
    try {
        ...
        const relatedEntities = extractRelatedEntities(searchResults, parameters.query);
        
        // Generate answer using LLM
        const answer = await generateAnswer(context, parameters.query, relevantContent, relatedEntities);
        ...
        console.log(`Knowledge query completed: ${sources.length} sources, ${relatedEntities.length} related entities`);
...
): Promise<{ isIndexed: boolean; lastIndexed: string | null; entityCount: number }> {
...
async function exportKnowledgeData(
    ...
    try {
        ...
        const exportData: any = {
            ...
            relationships: documents.flatMap(doc => doc.knowledge?.relationships || []),
        };
...
function extractRelatedEntities(documents: WebPageDocument[], query: string): Entity[] {
...
async function generateAnswer(
    ...
    try {
        // Use the real LLM translator
        const agent = await getKnowledgeAgent();
        const response = await agent.answerQuery(query, relevantContent, relatedEntities);
        return response;
    } catch (error) {

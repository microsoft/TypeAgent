...
export class EntityProcessingService {
     ...
     */
    async processWebsites(websites: any[], collection: WebsiteCollection): Promise<void> {
        if (!this.initialized) {
            await this.initialize();
        }

        try {
            debug(`Processing ${websites.length} websites for entity extraction`);
            ...
            const extractedEntities = await this.entityExtractor.extractEntitiesFromWebsites(websiteInputs);
            debug(`Extracted ${extractedEntities.length} entities`);

            // Convert extracted entities to canonical format
            const canonicalEntities = this.convertToCanonicalEntities(extractedEntities);
            ...
            const relationships = await this.entityExtractor.buildEntityRelationships(extractedEntities, websiteInputs);
     ...
     */
    private convertToCanonicalEntities(extractedEntities: any[]): EnhancedEntity[] {
        return extractedEntities.map(entity => ({
            ...
            coOccurringEntities: (entity.coOccurringEntities || []).map((coOcc: any) => ({
                entityName: coOcc.entity || coOcc.entityName || "",
                coOccurrenceCount: coOcc.coOccurrenceCount || 0,
                contexts: coOcc.contexts || [],
                confidence: coOcc.strength || coOcc.confidence || 0.5
     ...
     */
    private async storeEntitiesInCollection(entities: EnhancedEntity[], collection: WebsiteCollection): Promise<void> {
        for (const entity of entities) {
            ...
            const sourceUrls = entity.dominantDomains.map(domain => `https://${domain}`);
     ...
     */
    private async storeRelationships(entities: EnhancedEntity[], relationships: EntityRelationship[]): Promise<void> {
        for (const relationship of relationships) {
            ...
            if (sourceEntity) {
                this.graphManager.addRelationship(sourceEntity.name, relationship.relatedEntity, relationship);
            }
        }
    }
     ...
     */
    async getEntitiesByType(type: string, collection: WebsiteCollection): Promise<any[]> {
     ...
     */
    async getEntitiesByDomain(domain: string, collection: WebsiteCollection): Promise<any[]> {
        return (collection as any).knowledgeEntities.getEntitiesByDomain(domain);
    }

    /**
     * Search entities across the collection
     */
    async searchEntities(query: string, collection: WebsiteCollection): Promise<any[]> {

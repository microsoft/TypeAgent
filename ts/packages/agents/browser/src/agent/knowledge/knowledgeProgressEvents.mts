...
export interface KnowledgeExtractionProgressEvent {
    extractionId: string;
    phase: 'content' | 'basic' | 'summary' | 'analyzing' | 'extracting' | 'complete' | 'error';
    totalItems: number;
...
export type ProgressCallback = (progress: KnowledgeExtractionProgressEvent) => void | Promise<void>;
...
export class KnowledgeProgressEventEmitter extends EventEmitter {
    ...
    onProgress(listener: (progress: KnowledgeExtractionProgressEvent) => void): void {
        this.on('knowledgeExtractionProgress', listener);
    }

    onProgressById(extractionId: string, listener: (progress: KnowledgeExtractionProgressEvent) => void): void {
        this.on(`progress:${extractionId}`, listener);
    }

    onPhase(phase: KnowledgeExtractionProgressEvent['phase'], listener: (progress: KnowledgeExtractionProgressEvent) => void): void {
    ...
    getExtractionHistory(filter?: {
        ...
        if (filter) {
            if (filter.extractionId) {
                results = results.filter(event => event.extractionId === filter.extractionId);
            }
            if (filter.phase) {
                results = results.filter(event => event.phase === filter.phase);
            }
            if (filter.limit) {

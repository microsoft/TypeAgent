...
export class EnhancedDomainClassifier implements DomainClassifier {
    private domainPatterns = {
        documentation: [
            'docs', 'documentation', 'guide', 'manual', 'help', 'wiki',
            'github.io', 'readthedocs', 'gitbook'
        ],
        ecommerce: [
            'amazon', 'shop', 'store', 'buy', 'cart', 'checkout',
            'commerce', 'retail', 'marketplace'
        ],
        news: [
            'news', 'blog', 'article', 'post', 'media', 'journal',
            'times', 'herald', 'gazette', 'press'
        ],
        education: [
            'edu', 'university', 'college', 'school', 'course',
            'learn', 'tutorial', 'training', 'academy'
        ],
        social: [
            'social', 'forum', 'community', 'chat', 'message',
            'connect', 'network', 'share'
        ],
        technical: [
            'api', 'dev', 'developer', 'code', 'git', 'repo',
            'technical', 'engineering', 'software'
        ],
        reference: [
            'reference', 'lookup', 'search', 'find', 'directory',
            'index', 'catalog', 'database'
        ]
    };
    ...
    private extractKeywords(domain: string, category: string): string[] {
        ...
        domainParts.forEach(part => {
            if (part.length > 2 && !['www', 'com', 'org', 'net'].includes(part)) {
                keywords.push(part);
            }
        });
...
export class EnhancedTitleProcessor implements TitleProcessor {
    ...
    private calculateConfidence(title: string, keywords: string[], entities: string[]): number {
        let confidence = 0.5;

...
export class UnifiedKnowledgeExtractor {
    ...
    constructor(context: SessionContext<BrowserActionContext>) {
        ...
        this.metadataExtractor = new (class implements MetadataExtractor {
            ...
            extractFromUrl(url: string) {
                ...
                const domainInfo = this.domainClassifier.classify(
                    urlInfo.domain,
                );
                    ...
                    entities: [
                        {
                            ...
                            facets: [
                                ...
                                {
                                    name: "subdomain",
                                    value: urlInfo.subdomain || "www",
                                },
                                {
                                    name: "pathSegments",
                                    value: urlInfo.pathSegments.length.toString(),
                                },
                                {
                                    name: "category",
                                    value: domainInfo.category,
                                },
                            ],
                        },
                    ],
                    topics: [
                        ...urlInfo.pathSegments.filter(
                ...
                return {
                    entities: titleInfo.entities.map((entity) => ({
                        ...
                        facets: [
                            ...
                            {
                                name: "confidence",
                                value: titleInfo.confidence.toString(),
                            },
                        ],
                    })),
            ...
            extractTemporal(visitDate?: string, bookmarkDate?: string) {
                ...
                if (visitDate) {
                    ...
                    facets.visitYear = new Date(visitDate)
                        .getFullYear()
                        .toString();
                }
                if (bookmarkDate) {
                    facets.bookmarkDate = bookmarkDate;
                    facets.bookmarkYear = new Date(bookmarkDate)
                        .getFullYear()
                        .toString();
                }
                return facets;
            ...
            mergeWithAIKnowledge(
                ...
                return {
                    ...
                    topics: [
                        ...new Set([...metadata.topics, ...aiKnowledge.topics]),
                    ],
    ...
    private async extractMetadataKnowledge(
        content: ContentInput,
        ...
        return {
            ...
            topics: [
                ...new Set([...urlKnowledge.topics, ...titleKnowledge.topics]),
            ],
    ...
    private async extractAIKnowledge(
        ...
        try {
            const aiKnowledge =
                await this.aiModelManager.extractKnowledge(textContent);
            const metadataKnowledge =
                await this.extractMetadataKnowledge(content);
            ...
            return this.metadataExtractor.mergeWithAIKnowledge(
                metadataAsBasic,
                aiKnowledge,
            );
        } catch (error) {
            console.warn(
                "AI extraction failed, falling back to metadata:",
                error,
            );
            return this.extractMetadataKnowledge(content);
        }

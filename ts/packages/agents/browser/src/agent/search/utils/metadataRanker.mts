...
export class MetadataRanker {
     ...
     */
    async rankByAnalysis(results: Website[], analysis: QueryAnalysis): Promise<Website[]> {
        if (!analysis.ranking) {
            debug("No ranking requirements in analysis, using semantic order");
            return results;
        }

        debug(`Ranking ${results.length} results by ${analysis.ranking.primaryFactor} (${analysis.ranking.direction})`);
    ...
    private rankByDate(results: Website[], analysis: QueryAnalysis): Website[] {
        ...
        return results.sort((a, b) => {
            ...
            const comparison = analysis.ranking?.direction === "ascending" 
    ...
    private rankByFrequency(results: Website[], analysis: QueryAnalysis): Website[] {
    ...
    private rankByComposite(results: Website[], analysis: QueryAnalysis): Website[] {
    ...
    private getCompositeWeights(analysis: QueryAnalysis): { date: number; frequency: number; knowledge: number } {

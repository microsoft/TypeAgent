...
export class ListCommandHandler implements CommandHandlerNoParams {
    ...
    public async run(
        ...
        const activeSearchProvider: SearchProvider =
            context.sessionContext.agentContext.activeSearchProvider;

        searchProviders.forEach((provider) => {
            if (
                provider.name.toLowerCase() ===
                activeSearchProvider.name.toLowerCase()
            ) {
...
export class SetCommandHandler implements CommandHandler {
    ...
    public readonly parameters = {
        args: {
            provider: {
                description:
                    "The name of the search provider to set as active.",
            },
        },
    } as const;
    public async run(
        ...
        searchProviders.forEach((provider) => {
            if (
                ...
                context.sessionContext.agentContext.activeSearchProvider =
                    provider;
                displayResult(
                    `${provider.name} is now the active search provider.`,
                    context,
                );
                bFound = true;
...
export class AddCommandHandler implements CommandHandler {
    ...
    public readonly parameters = {
        args: {
            ...
            url: {
                description:
                    "The URL of the search provider to add. '%s' will be replaced with the search parameter.",
            },
        },
    } as const;
    public async run(
        ...
        if (params.args.url.indexOf("%s") === -1) {
            displayError(
                `The URL must contain '%s' as a placeholder for the search parameter.`,
                context,
            );
            return;
        }
        ...
        const idx = searchProviders.findIndex(
            (provider) =>
                provider.name.toLowerCase() ===
                params.args.provider.toLowerCase(),
        );
        if (idx !== -1) {
            displayError(
                `A search provider with the name '${params.args.provider}' already exists.`,
                context,
            );
        } else {
            ...
            displayResult(
                `Added search provider '${params.args.provider}'.`,
                context,
            );
        }
    }
...
export class RemoveCommandHandler implements CommandHandler {
    ...
    public async run(
        ...
        const newSearchProviders = searchProviders.filter(
            (value) =>
                value.name.toLowerCase() !== params.args.provider.toLowerCase(),
        );

        if (providerCount > newSearchProviders.length) {
            if (newSearchProviders.length === 0) {
                displayError(
                    "You cannot delete the search provider.  There must be at least one search provider.",
                    context,
                );
                return;
            }
            ...
            if (
                context.sessionContext.agentContext.activeSearchProvider.name.toLowerCase() ===
                params.args.provider.toLowerCase()
            ) {
                context.sessionContext.agentContext.activeSearchProvider =
                    newSearchProviders[0];
                displayResult(
                    `Search provider '${params.args.provider}' was the active search provider. New active search provider: ${newSearchProviders[0].name}`,
                    context,
                );
            }
            ...
            displayResult(
                `Search provider '${params.args.provider}' removed.`,
                context,
            );
        } else {
            displayError(
...
export class ImportCommandHandler implements CommandHandler {
    ...
    public readonly parameters = {
        args: {
            browser: {
                description:
                    "The name of the browser to import search providers from: [Edge | Chrome].",
            },
        },
    } as const;
    public async run(

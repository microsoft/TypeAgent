...
class ActionIndexPanel {
    ...
    private setupEventListeners() {
        ...
        // View mode controls
        document.getElementById("viewModeWebsite")!.addEventListener("click", () => {
            this.setViewMode('website');
        });

        document.getElementById("viewModeCategory")!.addEventListener("click", () => {
            this.setViewMode('category');
        });

        document.getElementById("viewModeUsage")!.addEventListener("click", () => {
            this.setViewMode('usage');
        });

        document.getElementById("viewModeTimeline")!.addEventListener("click", () => {
        ...
        document.getElementById("dateRangeFilter")!.addEventListener("change", (e) => {
            this.state.filters.dateRange = (e.target as HTMLSelectElement).value;
            this.applyFilters();
        });

        document.getElementById("usageFrequencyFilter")!.addEventListener("change", (e) => {
            this.state.filters.usageFrequency = (e.target as HTMLSelectElement).value;
            this.applyFilters();
        });
    ...
    private updateViewModeUI() {
        ...
        const buttons = ['viewModeWebsite', 'viewModeCategory', 'viewModeUsage', 'viewModeTimeline'];
    ...
    private updateActionSelectionUI() {
        ...
        this.state.filteredActions.forEach(action => {
            const checkbox = document.querySelector(`[data-action-checkbox="${action.id || action.name}"]`) as HTMLInputElement;
            if (checkbox) {
        ...
        const selectedCountElement = document.getElementById("selectedActionsCount");
        if (selectedCountElement) {
            selectedCountElement.textContent = this.state.selectedActions.length.toString();
        }
    }
    ...
    private async bulkDeleteActions() {
        ...
        deleteButton.innerHTML = '<span class="spinner-border spinner-border-sm" role="status"></span> Deleting...';
    ...
    private clearFilters() {
        ...
        (document.getElementById("dateRangeFilter") as HTMLSelectElement).value = "all";
        (document.getElementById("usageFrequencyFilter") as HTMLSelectElement).value = "all";
    ...
    private populateUsageFrequencyFilter() {
        const usageFilter = document.getElementById("usageFrequencyFilter") as HTMLSelectElement;
    ...
    private applyFilters() {
        ...
        if (this.state.filters.dateRange !== "all") {
            filtered = filtered.filter(action => {
                ...
                const now = new Date();
                const filterDate = this.getDateRangeStart(this.state.filters.dateRange, now);
        ...
        if (this.state.filters.usageFrequency !== "all") {
            filtered = filtered.filter(action => {
                ...
                return this.matchesUsageFrequency(usageStats, this.state.filters.usageFrequency);
            });
        }
    ...
    private getActionUsageStats(action: any): { count: number; lastUsed: Date | null } {
        ...
        const stats = this.state.actionStatistics.find(stat => 
    ...
    private renderByCategory(container: HTMLElement) {
        ...
        Object.entries(groupedActions).forEach(([category, actions]) => {
            const groupCard = this.createGroupCard(category, actions, 'category');
            container.appendChild(groupCard);
        });
    }
    ...
    private groupActionsByUsageFrequency(sortedActions: any[]): Record<string, any[]> {
        const grouped: Record<string, any[]> = {
            "Frequently Used": [],
            "Occasionally Used": [],
            "Rarely Used": [],
        ...
        sortedActions.forEach(action => {
            ...
            const daysSinceLastUse = stats.lastUsed ? 
                Math.floor((now.getTime() - stats.lastUsed.getTime()) / (1000 * 60 * 60 * 24)) : 999;
            
            if (stats.count >= 10 || (stats.count > 0 && daysSinceLastUse <= 7)) {
                grouped["Frequently Used"].push(action);
            } else if (stats.count >= 3 && stats.count < 10 && daysSinceLastUse <= 30) {
                grouped["Occasionally Used"].push(action);
            } else if (stats.count > 0 && stats.count < 3 && daysSinceLastUse > 30) {
    ...
    private groupActionsByTimeline(): Record<string, any[]> {
        ...
        const periods = ["Today", "This Week", "This Month", "Last 3 Months", "This Year", "Older", "Unknown"];
    ...
    private createGroupCard(groupName: string, actions: any[], groupType: string): HTMLElement {
        ...
        const selectCheckbox = groupCard.querySelector('.group-select-checkbox') as HTMLInputElement;
        selectCheckbox?.addEventListener('change', (e) => {
            this.toggleGroupSelection(groupName, actions, (e.target as HTMLInputElement).checked);
        });
    ...
    private createGroupSelectCheckbox(groupName: string, actions: any[]): string {
        const allSelected = actions.every(action => this.state.selectedActions.includes(action.id || action.name));
        const someSelected = actions.some(action => this.state.selectedActions.includes(action.id || action.name));
    ...
    private createGroupActionCard(action: any, index: number, groupName: string): string {
        const domain = this.extractDomain(action);
        const category = this.getActionCategory(action);
        const stats = this.getActionUsageStats(action);
        const isSelected = this.state.selectedActions.includes(action.id || action.name);
    ...
    private toggleGroup(groupName: string) {
        ...
        const toggleButton = document.querySelector(`[data-group-name="${groupName}"] .group-toggle i`);
        if (toggleButton) {
    ...
    private toggleGroupSelection(groupName: string, actions: any[], selected: boolean) {
        ...
        if (selected) {
            ...
            this.state.selectedActions = this.state.selectedActions.filter(id => !actionIds.includes(id));
        }
    ...
    private updateGroupCheckboxes() {
        document.querySelectorAll('.group-select-checkbox').forEach(checkbox => {
            ...
            if (groupHeader) {
                const groupContent = groupHeader.parentElement?.querySelector('.group-content');
                if (groupContent) {
                    const actionCheckboxes = groupContent.querySelectorAll('.action-checkbox') as NodeListOf<HTMLInputElement>;

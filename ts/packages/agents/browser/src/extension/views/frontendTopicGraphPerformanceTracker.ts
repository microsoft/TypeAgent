...
export class FrontendTopicGraphPerformanceTracker {
    ...
    endOperation(): FrontendTopicGraphMetrics | null {
        ...
        console.log(
            `[TopicGraphView] Completed operation: ${this.currentOperation}`,
        );
        console.log(
            `[TopicGraphView] Total duration: ${result.totalDuration.toFixed(2)}ms`,
        );
        console.log(
            `[TopicGraphView] Data fetch: ${result.dataFetchTime.toFixed(2)}ms`,
        );
        console.log(
            `[TopicGraphView] Data processing: ${result.dataProcessingTime.toFixed(2)}ms`,
        );
        console.log(
            `[TopicGraphView] DOM render: ${result.domRenderTime.toFixed(2)}ms`,
        );
        console.log(
            `[TopicGraphView] Cytoscape render: ${result.cytoscapeRenderTime.toFixed(2)}ms`,
        );
        console.log(
            `[TopicGraphView] Topics fetched: ${result.itemCounts.topicsFetched}, rendered: ${result.itemCounts.topicsRendered}`,
        );
        console.log(
            `[TopicGraphView] Relationships fetched: ${result.itemCounts.relationshipsFetched}, rendered: ${result.itemCounts.relationshipsRendered}`,
        );
    ...
    private printPerformanceWarnings(metrics: FrontendTopicGraphMetrics): void {
        ...
        console.warn(
            `  Data Processing: ${metrics.dataProcessingTime.toFixed(2)}ms (${((metrics.dataProcessingTime / metrics.totalDuration) * 100).toFixed(1)}%)`,
        );
        console.warn(
            `  DOM Render: ${metrics.domRenderTime.toFixed(2)}ms (${((metrics.domRenderTime / metrics.totalDuration) * 100).toFixed(1)}%)`,
        );
        console.warn(
            `  Cytoscape Render: ${metrics.cytoscapeRenderTime.toFixed(2)}ms (${((metrics.cytoscapeRenderTime / metrics.totalDuration) * 100).toFixed(1)}%)`,
        );
        ...
        if (metrics.dataFetchTime > metrics.totalDuration * 0.4) {
            console.warn(
                `  - Data fetching is slow (${((metrics.dataFetchTime / metrics.totalDuration) * 100).toFixed(1)}% of time)`,
            );
            console.warn(
                `  - Consider implementing request caching or reducing data volume`,
            );
        }

        if (metrics.dataProcessingTime > metrics.totalDuration * 0.3) {
            console.warn(
                `  - Data processing is slow (${((metrics.dataProcessingTime / metrics.totalDuration) * 100).toFixed(1)}% of time)`,
            );
            console.warn(
                `  - Consider optimizing data transformation algorithms`,
            );
        }

        if (metrics.cytoscapeRenderTime > metrics.totalDuration * 0.4) {
            console.warn(
                `  - Cytoscape rendering is slow (${((metrics.cytoscapeRenderTime / metrics.totalDuration) * 100).toFixed(1)}% of time)`,
            );
            console.warn(
                `  - Consider reducing node/edge count or simplifying layout`,
            );
        }

        if (metrics.itemCounts.topicsFetched > 1000) {
            console.warn(
                `  - Large number of topics (${metrics.itemCounts.topicsFetched}) may impact performance`,
            );
            console.warn(
                `  - Consider implementing pagination or hierarchical loading`,
            );
        }

        if (metrics.itemCounts.domUpdates > 50) {
            console.warn(
                `  - High number of DOM updates (${metrics.itemCounts.domUpdates}) may cause layout thrashing`,
            );
            console.warn(
                `  - Consider batching DOM updates or using virtual scrolling`,
            );
        }

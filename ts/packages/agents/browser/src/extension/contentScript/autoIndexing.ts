...
class AutoIndexingManager {
    ...
    private settingsChanged(changes: { [key: string]: chrome.storage.StorageChange }): boolean {
        const relevantKeys = ['autoIndexing', 'indexingDelay', 'excludeSensitiveSites', 'indexingQuality', 'indexOnlyTextContent'];
    ...
    private setupNavigationListeners() {
        ...
        window.addEventListener('replacestate', () => this.onNavigationChange());
    ...
    private async checkForAutoIndex() {
        ...
        if (this.settings.excludeSensitiveSites && this.isSensitiveSite(currentUrl)) {
            console.log("Skipping auto-indexing for sensitive site:", currentUrl);
            return;
        }
    ...
    private isSensitiveSite(url: string): boolean {
        const sensitivePatterns = [
            ...
            /banking/i, /bank\./i, /credit.*union/i, /paypal/i, /payment/i, 
            /checkout/i, /billing/i, /invoice/i, /financial/i,
            
            // Authentication and personal
            /login/i, /signin/i, /auth/i, /password/i, /reset/i,
            /account/i, /profile/i, /settings/i, /preferences/i,
            
            // Healthcare and medical
            /healthcare/i, /medical/i, /patient/i, /health/i, /doctor/i,
            /clinic/i, /hospital/i, /pharmacy/i,
            
            // Admin and internal
            /admin/i, /dashboard/i, /management/i, /internal/i,
            /intranet/i, /localhost/i, /127\.0\.0\.1/, /192\.168\./,
            /\.local/i, /staging/i, /test/i, /dev/i,
            
            // Social media private areas
            /messages/i, /chat/i, /direct/i, /private/i,
        ];
    ...
    private shouldIndexUrl(url: string): boolean {
        ...
        const excludePatterns = [
            /\/search\?/i, /\/results\?/i, /\/temp\//i, /\/tmp\//i,
            /\/api\//i, /\/ajax\//i, /\/xhr\//i,
        ];
    ...
    private async performAutoIndex() {
        ...
        try {
            ...
            if (response?.success) {
                console.log("Auto-indexing completed for:", window.location.href);
            } else {

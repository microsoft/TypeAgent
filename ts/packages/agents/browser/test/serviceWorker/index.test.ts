// test/serviceWorker/index.test.ts
...
// Import the module we want to test - this needs to happen AFTER the mocks are set up
...
describe('Service Worker initialization', () => {
  beforeEach(() => {
    ...
    jest.isolateModules(() => {
      ...
      ensureWebsocketConnected = websocketModule.ensureWebsocketConnected as jest.Mock;
      reconnectWebSocket = websocketModule.reconnectWebSocket as jest.Mock;
      showBadgeError = uiModule.showBadgeError as jest.Mock;
      initializeContextMenu = contextMenuModule.initializeContextMenu as jest.Mock;
  ...
  it('should handle errors during websocket connection', async () => {
    ...
    ensureWebsocketConnected.mockRejectedValue(new Error('Connection failed'));

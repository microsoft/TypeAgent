...
describe("Explorer action dispatcher", () => {
    ...
    it("discovers and executes Explorer actions through the canonical dispatcher", async () => {
        ...
        try {
            ...
            expect(schema).not.toContain("excludedReadCallIndices");
    ...
    it("grounds a range covered by consecutive visible grep lines only", async () => {
        ...
        try {
            ...
            const context = session.submissionContext();
            expect(context).toContain('[grep callIndex=0 path="src/alpha.ts"]');
            expect(context).toContain("1\texport function alpha() {");
            expect(context).toContain("2\t    return 'needle';");
    ...
    it("preserves historical refinement reads below the 1000-line host cap", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
    ...
    it("preserves the complete bounded discovery window below the historical action-result cap", async () => {
        const runtime = await createExplorerActionDispatcher(
            await createSession(),
        );
        try {
            const discovery = await runtime.executeAction(
                "explorer",
                "discoverRepository",
                { program: longContextDiscoveryProgram() },
            );
            expect(discovery).toMatchObject({ isError: false });
            const payload = JSON.parse(discovery.text) as {
                observations: Array<{ lines: string[] }>;
            };
            expect(discovery.text.length).toBeLessThanOrEqual(20_000);
            expect(
    ...
    it("preserves both complete bounded refinement reads below the historical action-result cap", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
            ...
            const refinement = await runtime.executeAction(
                "explorer",
                "refineRepository",
                { program: longContextRefinementProgram() },
            );
            expect(refinement).toMatchObject({ isError: false });
            ...
            expect(payload).not.toHaveProperty("submissionEvidence");
        } finally {
            await runtime.close();
        }
    });
    ...
    it("allows all remaining refinement calls to use targeted greps", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
            ...
            await expect(
                runtime.executeAction("explorer", "refineRepository", {
                    program: threeGrepsAndReadProgram(),
                }),
            ).resolves.toMatchObject({ isError: false });
            expect(
                session.snapshot().toolTrace.calls.map((call) => call.tool),
            ).toEqual(["grep", "grep", "grep", "grep", "read"]);
        } finally {
            await runtime.close();
        }
    });
    ...
    it("grounds bounded context around a path-scoped refinement grep", async () => {
        ...
        try {
            ...
            expect(refinement).toMatchObject({ isError: false });
            expect(refinement.text).toContain("mutation-marker");
            expect(session.submissionContext()).toContain(
                "194\t    return mutation-marker;",
            );
    ...
    it("keeps later discovery evidence visible when an early read reaches the response cap", async () => {
        const runtime = await createExplorerActionDispatcher(
            await createSession(),
        );
        try {
            const discovery = await runtime.executeAction(
                "explorer",
                "discoverRepository",
                { program: earlyHugeReadThenEvidenceProgram() },
            );
            expect(discovery).toMatchObject({ isError: false });
            expect(discovery.text.length).toBeLessThanOrEqual(20_000);
            const payload = JSON.parse(discovery.text) as {
            ...
            expect(
                payload.observations
                    .filter(
                        (observation) => observation.path === "src/alpha.ts",
                    )
                    .flatMap((observation) => observation.lines),
            ).toEqual(
                expect.arrayContaining([expect.stringContaining("needle")]),
            );
        } finally {
            await runtime.close();
        }
    ...
    it("keeps raw read evidence groundable after response compaction", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            const discovery = await runtime.executeAction(
            ...
            expect(discovery).toMatchObject({ isError: false });
            expect(discovery.text.length).toBeLessThanOrEqual(40_000);
            const payload = JSON.parse(discovery.text) as {
                observationsTruncated: boolean;
                observations: Array<{
                    path: string;
            ...
            const hiddenLine = Array.from(
                { length: 200 },
                (_, index) => index + 1,
            ).find((line) => !visibleLines.has(line));
            ...
            const refinement = await runtime.executeAction(
                ...
                "refineRepository",
                {
                    program: hiddenDiscoveryCandidateProgram(hiddenLine!),
                },
            );
            expect(refinement).toMatchObject({ isError: false });
            ...
            expect(refinementPayload.submissionEvidence).toContain(
                `${hiddenLine}\t${String(hiddenLine).padStart(3, "0")}-${"x".repeat(496)}`,
            );
    ...
    it("rejects raw evidence that was absent from the model-visible transcript", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            const discovery = await runtime.executeAction(
            ...
            for (const result of [discovery, refinement]) {
                const payload = JSON.parse(result.text) as {
                    observations: Array<{ path: string; lines: string[] }>;
                    submissionEvidence?: string;
                };
                for (const line of payload.observations
                    .filter((observation) => observation.path === "src/huge.ts")
                    .flatMap((observation) => observation.lines)) {
                    visibleLineNumbers.add(Number(line.split("\t", 1)[0]));
                }
                for (const line of payload.submissionEvidence?.split("\n") ??
                    []) {
                    if (/^\d+\t/u.test(line)) {
                        visibleLineNumbers.add(Number(line.split("\t", 1)[0]));
                    }
            ...
            const hiddenLine = Array.from(
                { length: 200 },
                (_, index) => index + 1,
            ).find((line) => !visibleLineNumbers.has(line));
            expect(hiddenLine).toBeDefined();
    ...
    it("merges discovery and refinement advisory candidates for submission", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
                runtime.executeAction("explorer", "discoverRepository", {
                    program: discoveryCandidateProgram(),
                }),
            ).resolves.toMatchObject({ isError: false });
            await expect(
                runtime.executeAction("explorer", "refineRepository", {
                    program: refinementCandidateProgram(),
                }),
            ).resolves.toMatchObject({ isError: false });

            const candidateLine = session.submissionContext().split("\n")[1];
            expect(JSON.parse(candidateLine)).toEqual([
                { path: "src/wide.ts", startLine: 1, endLine: 1 },
    ...
    it("keeps oversized advisory candidate metadata valid JSON", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
                runtime.executeAction("explorer", "discoverRepository", {
                    program: manyCandidatesProgram("discover"),
                }),
            ).resolves.toMatchObject({ isError: false });
            await expect(
                runtime.executeAction("explorer", "refineRepository", {
                    program: manyCandidatesProgram("refine"),
                }),
            ).resolves.toMatchObject({ isError: false });

            const candidateLine = session.submissionContext().split("\n")[1];
            expect(JSON.parse(candidateLine)).toMatchObject({
                truncated: true,
                totalItems: 12,
            });
            expect(candidateLine.length).toBeLessThanOrEqual(4_000);
        } finally {
            await runtime.close();
        }
    });
    ...
    it("discards observations from a failed repository program", async () => {
        const session = await createSession();
        const runtime = await createExplorerActionDispatcher(session);
        try {
            await expect(
                runtime.executeAction("explorer", "discoverRepository", {
                    program: failedReadProgram(),
                }),
            ).resolves.toMatchObject({
                isError: true,
                text: expect.stringMatching(/discard this attempt/i),
            });
            expect(session.snapshot()).toMatchObject({
                programAttempts: 0,
                observationCount: 0,
                toolTrace: {
                    totalCalls: 1,
                    calls: [{ tool: "read", discarded: true }],
                },
            });
    ...
    async function createSession(
        ...
        await writeFile(
            path.join(repoRoot, "src", "long-context.ts"),
            Array.from(
                { length: 400 },
                (_, index) =>
                    `context-line-${String(index + 1).padStart(3, "0")}-${"x".repeat(48)}`,
            ).join("\n"),
        );
        await writeFile(
...
function earlyHugeReadThenEvidenceProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.read("src/huge.ts", { offset: 0, limit: 200 });
    await repo.grep("needle", { path: "src/alpha.ts", literal: true, maxMatches: 1 });
    await repo.read("src/alpha.ts", { offset: 0, limit: 3 });
    return { success: true, message: params.query };
}`;
}

function hiddenDiscoveryCandidateProgram(line: number): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.read("src/alpha.ts", { offset: 0, limit: 3 });
    return {
        ...
        locations: [{ path: "src/huge.ts", startLine: ${line}, endLine: ${line} }],
    };
}`;
}

function discoveryCandidateProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.read("src/alpha.ts", { offset: 0, limit: 3 });
    return {
        success: true,
        message: params.query,
        locations: [{ path: "src/alpha.ts", startLine: 1, endLine: 1 }],
    };
}`;
}

function refinementCandidateProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    ...
    return {
        success: true,
        message: params.query,
        locations: [{ path: "src/wide.ts", startLine: 1, endLine: 1 }],
    };
}`;
}

function manyCandidatesProgram(prefix: string): string {
    const locations = Array.from({ length: 6 }, (_, index) => ({
        path: `src/${prefix}-${index}-${"x".repeat(600)}.ts`,
        startLine: 1,
        endLine: 1,
    }));
    return `
...
function failedReadProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.read("src/alpha.ts", { offset: 0, limit: 3 });
    return { success: false, error: "discard this attempt", message: params.query };
}`;
}
...
function threeGrepsAndReadProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.grep("needle", { literal: true, path: "src/alpha.ts", maxMatches: 1 });
    await repo.grep("missing-one", { literal: true, path: "src/alpha.ts", maxMatches: 1 });
    await repo.grep("missing-two", { literal: true, path: "src/alpha.ts", maxMatches: 1 });
    await repo.read("src/alpha.ts", { offset: 0, limit: 3 });
    return { success: true, message: params.query };
}`;
}
...
function longContextDiscoveryProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    await repo.read("src/long-context.ts", { offset: 0, limit: 200 });
    return { success: true, message: params.query };
}`;
}

function longContextRefinementProgram(): string {
    return `
async function execute(repo: RepositoryApi, params: ExploreParams): Promise<ExploreProgramResult> {
    for (let offset = 0; offset < 400; offset += 200) {
        await repo.read("src/long-context.ts", { offset, limit: 200 });
    }
    return { success: true, message: params.query };
}`;
}

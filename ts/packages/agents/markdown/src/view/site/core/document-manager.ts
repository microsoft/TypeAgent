...
export class DocumentManager {
    ...
    private switchRequestGeneration = 0;
    ...
    private async performAutoSave(): Promise<void> {
        try {
            ...
            if (this.isBindingTransitionInProgress) {
                console.log(
                    "[AUTO-SAVE] Skipping - document binding is changing",
                );
                return;
            }
        ...
        switch (data.type) {
            case "documentChanged":
                ...
                if (
                    ...
                    console.warn(
                        "[SSE] Ignoring incomplete documentChanged binding",
                    );
                    break;
                }
            ...
            case "bindingBootstrap": {
                ...
                if (
                    ...
                    console.warn(
                        "[SSE] Ignoring incomplete bindingBootstrap event",
                    );
                }
    ...
    public async saveDocument(editor?: Editor): Promise<void> {
        try {
            if (this.isBindingTransitionInProgress) {
                throw new Error(
                    "Cannot save while the document binding is changing",
                );
            }
    ...
    public async switchToDocument(documentPath: string): Promise<void> {
        try {
            ...
            const requestGeneration = ++this.switchRequestGeneration;
            ...
            const relative =
                typeof result.boundRelativePath === "string"
                    ...
                    : typeof result.relativePath === "string"
                      ? result.relativePath
                      : null;
            if (requestGeneration !== this.switchRequestGeneration) {
                console.warn(
                    `[DOCUMENT] Ignoring superseded switch response for ${documentPath}`,
                );
                return;
            }
            if (
                ...
                (typeof result.bindingToken !== "string" ||
                    result.bindingToken !== this.currentBindingToken ||
                    relative !== this.currentBoundRelativePath)
            ) {
                console.warn(

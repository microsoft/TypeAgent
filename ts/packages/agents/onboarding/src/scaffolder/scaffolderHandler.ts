...
// Phase 5 ΓÇö Scaffolder handler.
...
import type {
    ApiSurface,
    DiscoveredAction,
} from "../discovery/discoveryHandler.js";
...
async function handleListTemplates(): Promise<ActionResult> {
    const lines = [
        ...
        ``,
        `### Agent templates`,
        `- **default** ΓÇö TypeAgent agent package (manifest, handler, schema, grammar)`,
        ``,
        `### Plugin templates (use with \`scaffoldPlugin\`)`,
        ...Object.entries(PLUGIN_TEMPLATES).map(
            ([key, t]) => `- **${key}** ΓÇö ${t.description}`,
        ),
    ];
...
// ΓöÇΓöÇΓöÇ Pattern listing ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function handleListPatterns(): Promise<ActionResult> {
    const lines = [
        ...
        `| \`external-api\` | REST/OAuth cloud API (MS Graph, Spotify, GitHubΓÇª) | calendar, email, player |`,
...
// Pattern: external-api ΓÇö REST/OAuth cloud API bridge.
...
// Pattern: llm-streaming ΓÇö LLM-injected agent with streaming responses.
...
// Pattern: sub-agent-orchestrator ΓÇö root agent routing to N typed sub-schemas.
...
// Pattern: websocket-bridge ΓÇö bidirectional RPC to a host-side plugin.
// The agent owns a WebSocketServer; the host plugin connects as the client.
// Commands flow TypeAgent ΓåÆ WebSocket ΓåÆ plugin ΓåÆ response.
...
// Pattern: state-machine ΓÇö multi-phase disk-persisted workflow.
...
// Pattern: native-platform ΓÇö OS/device APIs via child_process or SDK.
...
// Pattern: view-ui ΓÇö web view renderer with IPC handler.
...
async function executeAction(
    ...
    return createActionResultFromHtmlDisplay(
        \`<p>Executing \${action.actionName} ΓÇö not yet implemented.</p>\`,
    );
}
`;
}
...
// Pattern: command-handler ΓÇö direct dispatch via a handlers map.

...
import { exitApplication, getAppPath, sendUserRequestAndWaitForResponse, startShell } from "./testHelper";
...
test.skip("why is the sky blue?", { tag: '@smoke' }, async ({}, testInfo) => {
    ...
    const msg = await sendUserRequestAndWaitForResponse(`why is the sky blue?`, mainWindow);

    expect(msg.toLowerCase(), "Chat agent didn't respond with the expected message.").toContain("raleigh scattering.")

...
export class TidyTree {
    ...
    public update(data: any): void {
            ...
            const root = this.config.path != null ? stratify().path(this.config.path)(data)
                : this.config.id != null || this.config.parentId != null ? stratify().id(this.config.id).parentId(this.config.parentId)(data)
            ...
            const L = this.config.label == null ? null : descendants.map(d => this.config.label!(d.data, d));
            ...
            if (this.config.height === undefined) this.config.height = x1 - x0 + dx * 2;
          
            // make sure we have a curve
            if (typeof this.config.curve !== "function") throw new Error(`Unsupported curve`);
          
            const svg = create("svg")
                .attr("viewBox", [-dy * this.config.padding / 2, x0 - dx, this.config.width, this.config.height])
            ...
            const node = svg.append("g")
              ...
              .join("a")
                .attr("xlink:href", this.config.link == null ? null : d => this.config.link!(d.data, d))
            ...
            if (this.config.title != null)
                { 
                    node.append("title")
                    .text(d => this.config.title!(d.data, d));
                }
...
export const defaultTidyTreeConfig: TidyTreeConfigType = {
 ...
 export type TidyTreeConfigType = {
     ...
     label: ((data: any, parent?: any) => ((d: any, p?: any) => any | null)) | null,// given a node d, returns the display name

...
export class TangledTree {
    ...
    private constructTangleLayout(levels, options: TangledTreeOptions = { c: 0, bigc: 0 }): any {
        ...
        levels.forEach((l, i) => {
          ...
          l.forEach(n => {
            ...
            if (id in index) {
              ...
              index[id] = { id: id, parents: n.parents.slice(), level: i, span: i - min(n.parents, p => p.level) };
            }
            n.bundle = index[id];
          });
        ...
        nodes.forEach(n => {
          ...
          n.bundles.sort((a,b) => descending(max(a, d => d.span), max(b, d => d.span)))
      ...
      private renderChart(data2, options: TangledTreeOptions = { c: 0, bigc: 0}): string {

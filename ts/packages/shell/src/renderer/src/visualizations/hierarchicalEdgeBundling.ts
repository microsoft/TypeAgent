...
import { ascending, hierarchy, cluster, create, lineRadial, curveBundle, select, selectAll } from "d3";
...
export class HierarchicalEdgeBundling {
    ...
    private makeChart(data: any) {
        ...
        const root = tree(this.linkNodes(hierarchy(data)
        ...
        svg.append("g")
          ...
          .text(d => d.data.name)
          .each( (d, index, group) => { d.text = group[index]; })
          .on("mouseover", (event: any, d: any) => {
                link.style("mix-blend-mode", null);
                select(event.currentTarget).attr("font-weight", "bold");
                selectAll(d.incoming.map(d => d.path)).attr("stroke", HierarchicalEdgeBundling.colorin).raise();
                selectAll(d.incoming.map(([d]) => d.text)).attr("fill", HierarchicalEdgeBundling.colorin).attr("font-weight", "bold");
                selectAll(d.outgoing.map(d => d.path)).attr("stroke", HierarchicalEdgeBundling.colorout).raise();
                selectAll(d.outgoing.map(([, d]) => d.text)).attr("fill", HierarchicalEdgeBundling.colorout).attr("font-weight", "bold");
          })
          .on("mouseout", (event: any, d) => {
            ...
            selectAll(d.incoming.map(([d]) => d.text)).attr("fill", null).attr("font-weight", null);
            selectAll(d.outgoing.map(d => d.path)).attr("stroke", null);
            selectAll(d.outgoing.map(([, d]) => d.text)).attr("fill", null).attr("font-weight", null);
          })
  ...
  ${d.incoming.length} incoming`));
        ...
        const link = svg.append("g")
          ...
          .each( (d, index, group) => { d.path = group[index]; });

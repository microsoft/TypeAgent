...
type FeedbackHost = {
    container: HTMLElement; // the chat-message-container-agent div
    bodyDiv: HTMLElement; // the chat-message-body div
    headerDiv: HTMLElement; // the chat-timestamp-agent div
    messageDiv: HTMLElement; // the chat-message-content div, for "copy" source
};
...
/**
 * Builds the feedback affordance for one agent message bubble. Owns the
 * trigger DOM (action row / header thumbs / flag glyph) and the popover
 * card. The variant can be swapped at runtime — `setVariant()` rebuilds
 * the trigger DOM in place.
 */
export class FeedbackWidget {
    ...
    /** Switch variants at runtime without recreating the message. */
    ...
    /** Re-render to reflect a (possibly external) rating change. */
    ...
    private async onThumb(
        ...
        // Re-click same thumb clears (for thumbs-up); thumbs-down always
        // routes through the popover so the user can edit category/comment.
        if (rating === "up") {
    ...
    private async copyMessage(button: HTMLButtonElement) {
        ...
        // Brief visual confirmation: swap to a checkmark and update the
        // tooltip, then revert after ~1.4s so the row resets cleanly.
        button.replaceChildren(iconCheck());
        button.classList.add("feedback-copied");
        button.title = "Copied!";
        button.setAttribute("aria-label", "Copied to clipboard");
        setTimeout(() => {
            // Only revert if the user hasn't navigated away / variant changed
...
function positionPopover(el: HTMLElement, anchor: HTMLElement) {
    // Make sure the popover is in the DOM with its actual size before we
    // can measure it. Position absolutely against viewport then clamp.
    ...
    el.style.visibility = "";
    // Remove the placeholder append we did first
...
/** Anchored card shown on thumbs-down clicks / "Other → comment" path. */

...
describe("NFA Infrastructure", () => {
    ...
    describe("word type is exactly one token", () => {
        it("does not swallow tokens before a following literal", () => {
            // Regression: $(p:word) used to compile as token+ and absorb "for"
            // before a day literal, beating more specific "for $(date)" rules.
            const grammar = loadGrammarRules(
                "word-one-token",
                `<Start> = with $(p:word) friday -> { p };`,
            );
            const nfa = compileGrammarToNFA(grammar, "word-one-token");

            expect(matchNFA(nfa, ["with", "alex", "friday"]).matched).toBe(
                true,
            );
            expect(matchNFA(nfa, ["with", "alex", "friday"]).actionValue).toEqual(
                { p: "alex" },
            );
            expect(
                matchNFA(nfa, ["with", "alex", "for", "friday"]).matched,
            ).toBe(false);
        });

        it("lets longer schedule alternatives win over short at $(time:word)", () => {
            const grammar = loadGrammarRules(
                "schedule-word",
                `<Start> =
  schedule a $(description:wildcard) for $(date:wildcard) at $(time:word) in $(location:wildcard) with $(participant:wildcard) -> {
    actionName: "scheduleEvent",
    parameters: { description, date, time, location, participant }
  }
  | schedule a $(description:wildcard) for $(date:wildcard) at $(time:word) -> {
    actionName: "scheduleEvent",
    parameters: { description, date, time }
  };`,
            );
            const nfa = compileGrammarToNFA(grammar, "schedule-word");

            const full = matchNFA(nfa, [
                "schedule",
                "a",
                "team",
                "meeting",
            ...
            expect(full.actionValue).toEqual({
                actionName: "scheduleEvent",
                parameters: {
            ...
            expect(short.actionValue).toEqual({
                actionName: "scheduleEvent",
                parameters: {
                    description: "team meeting",
                    date: "Friday",
    ...
    describe("book multi-word participant + RelativeDay", () => {
        const bookGrammar = `
...
<Start> =
  ...
  | book $(description:wildcard) with $(participant:wildcard) for $(date:wildcard) at $(time:word) -> {
    actionName: "scheduleEvent",
    parameters: { description, date, time, participant }
  }
  | book $(description:wildcard) with $(participant:wildcard) on $(date:wildcard) at $(time:word) -> {
    actionName: "scheduleEvent",
    parameters: { description, date, time, participant }
  }
...
`;
        ...
        it("binds multi-word participant before bare RelativeDay", () => {
            const grammar = loadGrammarRules("book-multi", bookGrammar);
            const nfa = compileGrammarToNFA(grammar, "book-multi");
            ...
            expect(result.matched).toBe(true);
            expect(result.actionValue).toEqual({
                actionName: "scheduleEvent",
                parameters: {
                    description: "sync",
                    date: "friday",
        ...
        it("binds multi-word participant with for/on separators", () => {
            const grammar = loadGrammarRules("book-multi", bookGrammar);
            const nfa = compileGrammarToNFA(grammar, "book-multi");
            ...
            expect(
                ...
                parameters: {
                    ...
                    date: "monday",
                    time: "3pm",
                    participant: "the whole team",
                },
            });
        });

        it("does not steal at-location book patterns", () => {
            const grammar = loadGrammarRules("book-multi", bookGrammar);
            const nfa = compileGrammarToNFA(grammar, "book-multi");
            const result = matchNFA(nfa, [
                "book",
        ...
        it("keeps single-token participant bare-day matches", () => {
            const grammar = loadGrammarRules("book-multi", bookGrammar);
            const nfa = compileGrammarToNFA(grammar, "book-multi");
            const result = matchNFA(nfa, [
                "book",

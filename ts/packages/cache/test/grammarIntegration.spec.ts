...
<stop> = stop -> {
    ...
    parameters: {}
            ...
            if (
                completions &&
                completions.groups.flatMap((g) => g.completions).length > 0
            ) {
                const completionStrings = completions.groups
                    .flatMap((g) => g.completions)
                    .map((c) => c.toLowerCase());
...
<TrackTerm> = track | song;`;
            ...
            expect(completions!.groups.flatMap((g) => g.completions)).toContain(
                "by",
            );
        });
    });

...
import {
    toDroidCallBenchmarkCase,
    type DroidCallSourceRow,
} from "../src/translationBench/public_datasets/DroidCall/droidCallBenchmark.js";

const source: DroidCallSourceRow = {
    ...
    tools: [
        {
            ...
            arguments: { name: { type: "str", required: false } },
        },
        {
            name: "send",
...
describe("DroidCall benchmark conversion", () => {
    it("preserves dependencies and optional parameter scoring", () => {
        const row = toDroidCallBenchmarkCase(source, "test", 4);
    ...
    it("excludes answers without a corresponding tool", () => {
        expect(
            toDroidCallBenchmarkCase(
                { ...source, tools: source.tools.slice(0, 1) },
                "test",
                4,
            ),
        ).toBeUndefined();

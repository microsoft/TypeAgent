...
test("resolves revisions and handles retries, cancellation, and atomic output", async () => {
    ...
    const fetch = jest.fn<typeof globalThis.fetch>(async (input) => {
        ...
        if (url.hostname === "huggingface.co") {
            if (++hubAttempts === 1) throw new TypeError("network failure");
            if (hubAttempts === 2) return new Response(null, { status: 503 });
    ...
    expect(requests.find((url) => url.hostname === "huggingface.co")!.pathname).toMatch(/\/revision\/feature%2Fone$/);
    expect(requests.filter((url) => url.hostname !== "huggingface.co").every((url) => !url.searchParams.has("revision"))).toBe(true); expect(progress).toEqual([1, 2]);
    expect(await readFile(outputPath, "utf8")).toBe('{"id":1}\n{"id":2}\n');

    const mismatchDir = await directory();
    const mismatch = jest.fn<typeof globalThis.fetch>(async (input) => {
        const url = new URL(String(input));
        if (url.hostname === "huggingface.co") return Response.json({ sha: SHA });
        const offset = Number(url.searchParams.get("offset"));
    ...
    const transient = jest.fn<typeof globalThis.fetch>(async (input) => { if (new URL(String(input)).hostname === "huggingface.co") return Response.json({ sha: SHA }); queueMicrotask(() => controller.abort()); return new Response(null, { status: 503 }); });

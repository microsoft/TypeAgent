...
const CASES = [
    ...
    [
        "Split the active editor showing server.js to the right.",
        "code.splitEditor",
    ],
    [
        "Please close the editor pane I'm looking at.",
        "code.code-display.closeEditor",
    ],
    [
        "Open the editor's Markdown preview for the file I'm on in the current tab, not in a side pane.",
        "code.code-display.openMarkdownPreview",
    ],
    [
        "In the editor, open this Markdown file's preview to the side.",
        "code.code-display.openMarkdownPreviewToSide",
    ],
    [
        "Open the Explorer sidebar in the editor.",
        "code.code-display.showExplorer",
    ],
    ["Open the Output panel.", "code.code-display.showOutputPanel"],
    [
        "Open the find bar in the editor so I can search this file.",
        "code.code-display.showSearch",
    ],
    [
        "Open the Source Control view in the editor.",
        "code.code-display.showSourceControl",
    ],
    ...
    [
        "Show VS Code's Keyboard Shortcuts panel.",
        "code.code-general.showKeyboardShortcuts",
    ],
    [
        "In VS Code, open the file server.ts by exact name, only among .ts files, and don't look in generated output folders.",
        "code.code-workbench.workbenchOpenFile",
    ],
    [
        "The screen is too bright for this room—turn the display brightness down.",
        "desktop.AdjustScreenBrightness",
    ],
    [
        "Set my Windows theme to MidnightBlue using C:\\Users\\Maya\\Downloads\\MidnightBlue.theme.",
        "desktop.ApplyTheme",
    ],
    ["Please turn Bluetooth on for me.", "desktop.BluetoothToggle"],
    ["Please turn Wi‑Fi back on for this laptop.", "desktop.EnableWifi"],
    [
        "Please make the Windows text larger for menus and title bars only—set text size to 135%, not the overall display scale.",
        "desktop.SetTextSize",
    ],
    ["Please turn on airplane mode on this PC.", "desktop.ToggleAirplaneMode"],
    [
        "Please turn off desktop notifications for now.",
        "desktop.ToggleNotifications",
    ],
    [
        "That's great! How about turning on the screen magnifier for me?",
        "desktop.desktop-system.EnableMagnifier",
    ],
    ["Set repeat to this song only.", "localPlayer.repeat"],
    [
        "In the Windows Clock app, start my focus session now.",
        "windowsClock.setFocusSessionRunning",
    ],
    [
        "Please start the stopwatch in Clock.",
        "windowsClock.setStopwatchRunning",
    ],
    ["Switch the timer to compact view.", "windowsClock.setTimerViewMode"],
    [
        "Please display the image files beach-sunrise.jpg and boardwalk-night.png.",
        "chat.showImageFile",
    ],
    [
        'Please switch my editor theme to "Solarized Dark".',
        "code.changeColorScheme",
    ],
    ["Switch the editor layout to two columns.", "code.changeEditorLayout"],
    [
        "Could you split the last editor that's showing server.js over to the left?",
        "code.splitEditor",
    ],
    [
        "Open the preview for this Markdown file in the current editor pane.",
        "code.code-display.openMarkdownPreview",
    ],
    [
        "Open the Markdown preview to the side in the editor for the file I'm viewing.",
        "code.code-display.openMarkdownPreviewToSide",
    ],
    [
        "Thanks — now bring up the Settings window in VS Code.",
        "code.code-display.openSettings",
    ],
    ["Show the Explorer pane in my editor.", "code.code-display.showExplorer"],
    [
        "Open the Output panel so I can inspect the build logs.",
        "code.code-display.showOutputPanel",
    ],
    [
        "Open the Find box so I can search for text in the current file.",
        "code.code-display.showSearch",
    ],
    [
        "In the code editor, can you now show the Source Control sidebar?",
        "code.code-display.showSourceControl",
    ],
    [
        "Could you switch the editor into Zen Mode now?",
        "code.code-display.zenMode",
    ],
    [
        "VS Code is acting weird after that extension change—reload the window for me.",
        "code.code-extension.reloadWindow",
    ],
    ["Go to line 128.", "code.code-general.gotoFileOrLineOrSymbol"],
    [
        "Open the Keyboard Shortcuts panel in VS Code.",
        "code.code-general.showKeyboardShortcuts",
    ],
    [
        "Please raise the screen brightness a notch.",
        "desktop.AdjustScreenBrightness",
    ],
    ["Go back to the theme I was using before.", "desktop.ApplyTheme"],
    [
        "Set the Windows accessibility text size setting to 125%, and do not change display scaling, resolution, or any app zoom.",
        "desktop.SetTextSize",
    ],
    ["Turn on airplane mode on this computer.", "desktop.ToggleAirplaneMode"],
    [
        "Please turn off desktop notifications for me.",
        "desktop.ToggleNotifications",
    ],
    [
        "Make the mouse pointer bigger so it's easier to see on this monitor.",
        "desktop.desktop-input.AdjustMousePointerSize",
    ],
    [
        "Turn on Filter Keys so the keyboard ignores brief or repeated key presses.",
        "desktop.desktop-system.EnableFilterKeysAction",
    ],
    [
        "Set quiet hours from 10 PM until 6 AM every night.",
        "desktop.desktop-system.EnableQuietHours",
    ],
    [
        "Please turn on the setting that minimizes my windows when a monitor gets disconnected.",
        "desktop.desktop-system.MinimizeWindowsOnMonitorDisconnectAction",
    ],
    [
        "Please log me out of GitHub on ghe.acme.internal.",
        "github-cli.authLogout",
    ],
...
for (const testCase of CASES)
    testCase.history = HISTORY_BY_UTTERANCE.get(testCase.utterance);
...
function configureModel() {
    ...
    if (!key)
        throw new Error("OPENAI_API_KEY or LOCAL_LITELLM_API_KEY is required");
    const base = (
        process.env.OPENAI_BASE_URL ?? "http://127.0.0.1:4627/v1"
    ).replace(/\/$/, "");
    process.env.OPENAI_API_KEY = key;
    process.env.OPENAI_ENDPOINT = `${base}/chat/completions`;
    process.env.OPENAI_MODEL = MODEL;
    process.env.OPENAI_MODEL_WIRE_API = JSON.stringify({
        [MODEL]: "responses",
    });
    initRuntimeConfigFromProcessEnv();
}
...
function createGoldProvider(base, allowlist) {
    ...
    for (const config of base.getActionConfigs()) {
        ...
        const actionSchemas = new Map(
            [...source.parsedActionSchema.actionSchemas].filter(
                ([actionName]) =>
    ...
    const agents = new Proxy(live.agents, {
        get(target, property) {
            if (property === "getActionConfig") {
                return (name) =>
                    provider.tryGetActionConfig(name) ??
                    target.getActionConfig(name);
            }
            if (property === "tryGetActionConfig") {
                return (name) =>
                    provider.tryGetActionConfig(name) ??
                    target.tryGetActionConfig(name);
            }
            const value = Reflect.get(target, property, target);
            return typeof value === "function" ? value.bind(target) : value;
        },
...
function format(actions) {
    return actions.length === 0
        ? "(no action)"
        : actions
              .map((action) => `${action.schemaName}.${action.actionName}`)
              .join(" + ");
}
...
const instanceDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "typeagent-luna-low-fnr-"),
);
...
try {
    ...
    console.log(
        `model=${MODEL} effort=low cases=${CASES.length} concurrency=${CONCURRENCY} goldActions=${goldAllowlist.size}`,
    );
    ...
    const misses = results.filter(
        (result) => !result.routed && result.error === undefined,
    );

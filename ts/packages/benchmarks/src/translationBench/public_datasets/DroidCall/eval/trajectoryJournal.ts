...
export interface DroidCallTrajectoryRecord {
    rowid: string;
    setupid: string;
    scenarioId: string;
    callIndex: number;
...
export function droidCallResponseText(response: unknown): string | undefined {
    if (typeof response !== "object" || response === null) return undefined;
    const data = (response as { data?: unknown }).data;
    return typeof data === "string" ? data : undefined;
}

export function reconcileDroidCallTrajectories(
    ...
    const records: DroidCallTrajectoryRecord[] = [];
    for (let index = 0; index < lines.length; index++) {
        const line = lines[index]!;
        if (line.length === 0) continue;
        try {
            records.push(JSON.parse(line) as DroidCallTrajectoryRecord);
        } catch (error) {
            const truncated =
    ...
    const unique = new Map<string, DroidCallTrajectoryRecord>();
    ...
    for (const record of reconciled) {
        ...
        const response = droidCallResponseText(record.response);

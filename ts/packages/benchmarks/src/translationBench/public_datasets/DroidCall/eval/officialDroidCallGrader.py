...
def score_payload(payload, contract_name):
    ...
    for row, response_map in zip(payload["rows"], prepared):
        ...
        for answer in row["answers"]:
            ...
            call_count += 1
        row_score = 0.0 if row_failed else (1.0 if row_total == 0 else row_correct / row_total)
    ...
    soft_accuracy = call_soft_total / call_count if aggregation == "function-call-mean" and call_count else row_soft_total / row_count if row_count else 0.0
    overrides = []
    ...
    return {
        ...
        "accuracy": perfect_rows / row_count if row_count else 0.0,
        "counts": {
            "rows": row_count,

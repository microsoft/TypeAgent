...
def deep_compare(left, right, match_type="strict", semantic_threshold=0.85):
    ...
    if is_field_none(left) and is_field_none(right):
        return True
    # Semantic pair collection skips one-sided sentinel values.
    if is_field_none(left) or is_field_none(right):
        return False
    if type(left) is not type(right):
        return False
    if isinstance(left, dict):
    ...
    if isinstance(left, list):
        if len(left) != len(right):
            return False
        # Consume each right-hand item once so duplicate values remain significant.
        ...
        for item in left:
            match = next(
                (
                    ...
                    if deep_compare(item, candidate, match_type, semantic_threshold)
                ),
                None,
            )
            if match is None:
...
def resolved_arguments(answer, response, api):
    for name, spec in api["arguments"].items():
        ...
        if not answer_has and not response_has:
            continue
        if spec.get("required", False) and not answer_has:
            continue
...
def score_payload(payload, contract_name):
    ...
    for row in payload["rows"]:
        # Ignore malformed response entries; they cannot represent a scored call.
        for response in row["response"]:
            if isinstance(response, dict) and isinstance(
                response.get("arguments"), dict
            ):
                response["arguments"] = decode_number_lexemes(
                    response["arguments"]
                )
    pairs = set()
    prepared = []
    ...
    soft_accuracy = (
        ...
        if aggregation == "function-call-mean" and call_count > 0
    ...
    return {
        ...
        "accuracy": perfect_rows / row_count if row_count > 0 else 0.0,
        "counts": {
            "rows": row_count,

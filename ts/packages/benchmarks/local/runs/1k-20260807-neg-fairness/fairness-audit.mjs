...
if (!available.includes(MODEL)) {
  throw new Error(`Model '${MODEL}' not configured. Available: ${available.join(", ")}`);
}
...
for (const line of lines) {
  ...
  for (const g of rec.generalizations || []) {
    ...
    negatives.push({
      ...
      target: targetStr,
      storedKind: sel.dimensions?.negativeKind || sel.dimensions?.kind || null,
...
if (SAMPLE && SAMPLE < negatives.length) {
  ...
  sample = Array.from({ length: SAMPLE }, (_, i) => negatives[Math.floor(i * step)]);
...
async function assessBatch(batch, offset) {
  ...
  try {
    ...
    throw new Error(`Bad JSON from audit model: ${String(result.data).slice(0, 400)}`);
  }

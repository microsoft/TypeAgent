...
for (const m of [GENERATOR_MODEL, REVIEWER_MODEL, ...AMBIGUITY_PROBE_MODELS]) {
...
function createTranslationBenchUsageAccumulator() {
    ...
    return {
        ...
        finish() {
            return {
                ...(hasBase ? { promptTokens, completionTokens } : {}),
...
function createGenerationLlm(modelName, role, limiter) {
    ...
    if (role === "generator") {
        modelConfiguration =
            promptsMod.loadTranslationBenchSynthesizerPromptPack()
                .modelConfiguration;
    } else {
        modelConfiguration =
            promptsMod.loadTranslationBenchQualityVerifierPromptPack()
                .semanticChecker.modelConfiguration;
    }
    const fromPrompt =
    ...
    return {
        ...
        async translate({ model, utterance, history, activeSchemas }) {
            try {
                ...
                if (
                    history !== undefined &&
                    disp.isChatHistoryInput?.(history)
                ) {
                ...
                const actions = translated.requestAction.actions.map(
                    (entry) => {
...
try {
    const result = await genMod.generateTranslationBenchBenchmark({
        ...
        requireCompleteCoverage:
            process.env.TB_REQUIRE_COMPLETE_COVERAGE !== "0",
        ...
        onProgress(completed, total, coverage) {
            ...
            const rate =
                completed > 0 ? (Number(elapsed) / completed).toFixed(1) : "?";
            const cov =
                coverage !== undefined

...
export function tbConfig() {
  ...
  const headroom = Number(process.env.TB_HEADROOM || evalCfg.headroom || 0.85);

  const generatorModel = process.env.TB_GENERATOR_MODEL || synth.generatorModel || "azure/gpt-5.4";
  const reviewerModel = process.env.TB_REVIEWER_MODEL || synth.reviewerModel || generatorModel;

  const genConcurrency = Number(
    process.env.TB_CONCURRENCY ||
      concurrencyFor(models[generatorModel], headroom, synth.concurrency || 20),
  );

  const evalModels = (process.env.TB_EVAL_MODELS
  ...
  const concurrencyByModel = Object.fromEntries(
    evalModels.map((id) => {
      ...
      const envOverride = process.env[`TB_CONC_${short}`] || process.env.TB_HIGH_CONCURRENCY;
      const c = envOverride
        ? Number(envOverride)
  ...
  return {
    ...
    maxAttempts: Number(process.env.TB_MAX_ATTEMPTS || synth.maxAttempts || 5),
    genConcurrency,
    evalModels,
    concurrencyByModel,
    modelConcurrency: Number(
      process.env.TB_MODEL_CONCURRENCY || evalCfg.modelConcurrency || evalModels.length || 1,
    ),
    maxCases:
      maxCasesRaw === null || maxCasesRaw === undefined ? undefined : Number(maxCasesRaw),

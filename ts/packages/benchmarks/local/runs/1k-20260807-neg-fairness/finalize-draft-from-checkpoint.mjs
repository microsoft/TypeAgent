...
const name =
    process.env.TB_BENCHMARK_NAME ||
    "typeagent-translation-bench-1k-all-actions";
...
for (const line of fs.readFileSync(ckpt, "utf8").split("\n")) {
    ...
    if (
        rec.kind !== "translation-bench-row" ||
        rec.value?.recordType !== "case"
    ) {
        continue;
    }
    const c = rec.value;
    if (
        typeof c.id !== "string" ||
        !c.seed ||
        !Array.isArray(c.generalizations)
    ) {
        throw new Error(

...
program
  ...
  .option("--runtime-ts <dir>", "sibling worktree ts/ root for dispatcher+runner")
  ...
  .option("--index-gateway-dir <dir>", "publish dir for the landing repo (index + explorers)")
...
try {
  ...
  const vizGwDir = cfg.indexGatewayDir || cfg.gatewayDir || path.join(RUN, "published");
  const { spawnSync } = await import("node:child_process");
  for (const gen of ["update-eval-cases-viz.mjs", "update-index-viz.mjs"]) {
    try {
      const r = spawnSync(
        process.execPath,
        [path.join(RUN, gen), "--art-dir", artDir, "--gateway-dir", vizGwDir],

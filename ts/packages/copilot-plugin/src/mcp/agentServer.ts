...
export class TypeAgentMcpServer {
    ...
    private readonly structuredClient: StructuredActionClient;

    constructor(structuredClient = createStructuredActionClient()) {
        ...
        this.registerTools();
        registerStructuredActionTools(this.server, this.structuredClient);
    }

    async close(): Promise<void> {
        await this.structuredClient.close();
    ...
    private registerTools(): void {
        this.server.registerTool(
            ...
            {
                title: "TypeAgent: Natural-language delegation (processCommand)",
                description:
    ...
    private async processCommand(
        ...
        try {
            ...
            const result = await submitCancellableCommand(
                dispatcher,
                command,
                extra?.signal,
            );
            ...
            return toolResult(
                "TypeAgent returned no display output. No completion is inferred from an empty response.",
            );
        } catch (error) {
            if (pendingPrompts.length > 0) {
                return toolResult(

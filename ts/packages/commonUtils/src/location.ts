...
import { GeoJsonFeature, GeoJsonFeatureCollection, SearchAddressResult, SearchAddressResultItem } from "@azure/maps-search";
...
export async function reverseGeocode(position: LatLong | undefined, settings: ApiSettings): Promise<ReverseGeocodeAddressLookup[]> {
    if (position === undefined) {
        return [];
    }
    ...
    try {
        ...
        // summarize results
        const results: GeoJsonFeatureCollection = responseBody as GeoJsonFeatureCollection;
        const retVal: ReverseGeocodeAddressLookup[] = [];
        results.features?.map((result: GeoJsonFeature) => {
            
            if (result.properties !== undefined) {
                retVal.push({ address: result.properties.address, confidence: result.properties.confidence, type: result.properties.type });
            }
        });

...
class Operation implements StructuredExecutionHooks {
    ...
    private readonly trackedActions = new Set<FullAction>();
    ...
    private contract(action: FullAction): ActionContract {
        ...
        const result = this.discovery.resolveActionContract(action);
        this.checkLive();
        ...
        if (result.envelope.scopeId !== this.request.scopeId) {
            throw new ExecutionFailure(
                "invalid_scope",
                "Action belongs to a different structured action scope",
            );
        }
        if (
            this.context.agents
    ...
    async guard(
        ...
        const approved = this.approved.get(action);
        if (
            approved?.effects !== contract.policy.effects ||
            approved?.confirmation !== contract.policy.confirmation
        ) {
            if (
            ...
            this.approved.set(action, contract.policy);
        }
        this.checkLive();
    }

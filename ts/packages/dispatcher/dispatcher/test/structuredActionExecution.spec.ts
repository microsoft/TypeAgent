...
describe("real structured dispatcher execution", () => {
    ...
    async function request(
        ...
        const search = await dispatcher.searchActions({
            query: `guarded ${actionName}`,
        });
        const contract = search.actions.find(
            (action) =>
                action.schemaName === "guarded" &&
                action.actionName === actionName,
        );
        if (contract === undefined) throw new Error("Missing fixture contract");
        return {
            protocolVersion: 1,
    ...
    it("reconfirms changed policy before invoking the handler", async () => {
        ...
        const reconfirm = await answer(prompt, {
            type: "confirmation",
            approved: true,
        });
        expect(reconfirm.status).toBe("requires_interaction");
        expect(entered).toEqual([]);
        expect(
            (
                await answer(reconfirm, {
                    type: "confirmation",
                    approved: true,
                })
            ).status,
    ...
    it.each(["contract_stale", "unavailable"] as const)(
        "retains %s from a nested typed-flow confirmation",
        async (status) => {
            const registry = (
                context.agents as unknown as {
    ...
    describe.each([false, true])(
        ...
        (rpc) => {
            it.each([
                ["question", "contract_stale"],
                ["question", "unavailable"],
                ["blockingForm", "contract_stale"],
                ["blockingForm", "unavailable"],
            ] as const)(
                "preserves %s -> %s without post-answer effects",
                async (mode, status) => {
    ...
    it.each<{ actionName: string; parameters: Record<string, unknown> }>([
        ...
        async ({ actionName, parameters }) => {
            ...
            expect(found.contract.availability.state).toBe("unsupported");
            const result = await dispatcher.executeAction({
            ...
            expect(
                search.actions.find((a) => a.actionName === "toggleAgent")
                    ?.availability.state,
            ).toBe("unsupported");
            expect(
                search.actions.find((a) => a.actionName === "listAgents")
                    ?.availability.state,
            ).toBe("available");
    ...
    it("rechecks policy after asynchronous entity preparation", async () => {
        holdResolution = true;
        const prompt = await dispatcher.executeAction(await request("resolve"));
        const result = answer(prompt, { type: "confirmation", approved: true });

...
| `--session <id>` | `-s`  | Session ID to use. Defaults to the `'CLI'` conversation if not specified.           |
...
- Pass `--session` / `-s <id>` to connect to any specific conversation by its UUID. Takes priority over `--resume` if both are provided.
- Pass `--memory` to use an ephemeral conversation that is created fresh and automatically deleted when you exit. Cannot be combined with `--session` or `--resume`.
...
`agent-cli sessions` provides full CRUD management of agent server conversations.
...
```bash
agent-cli sessions create                          # Created conversation 'CLI' (a1b2c3d4-e5f6-...)
agent-cli sessions create "workout playlist setup" # Created conversation 'workout playlist setup' (a1b2c3d4-e5f6-...)
```
...
```bash
agent-cli sessions delete a1b2c3d4-e5f6-...         # prompts: Delete conversation ...? (y/N)

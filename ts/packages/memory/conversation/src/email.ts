...
export async function createEmailCollection(
    fileSettings: IndexFileSettings,
    createNew: boolean,
): Promise<EmailCollection> {
    let em: EmailCollection | undefined;
    if (createNew) {
        ...
        em = await EmailCollection.readFromFile(fileSettings);
    }
    if (!em) {

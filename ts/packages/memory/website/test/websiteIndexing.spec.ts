...
describeIf(
    ...
    test("incremental indexing", async () => {
        ...
        const additionalWebsites = createTestWebsiteCollection(moreWebsites);
    ...
    test("knowledge extraction from website metadata", async () => {
        ...
        if (knowledge) {
            ...
            expect(knowledge.entities.some(e => e.name === "github.com")).toBe(true);
            
            // Should extract title as topic
            expect(knowledge.topics).toContain("TypeScript Programming Language");
            
            // Should extract folder as topic
            expect(knowledge.topics).toContain("Programming Languages");
            
            // Should extract action
            expect(knowledge.actions.some(a => 
    ...
    test("file persistence after indexing", async () => {
        ...
        const outputDir = await ensureOutputDir("test-results/websiteIndexing");
        await collection.writeToFile(outputDir, "persistenceTest");
        
        // Verify files were created
        const files = fs.readdirSync(outputDir);
        
        expect(files.some((f: string) => f.includes("persistenceTest"))).toBe(true);
    });
});
...
describeIf(
    ...
    () => {
        ...
        test(
            ...
            async () => {
                const collection = await loadTestWebsiteCollection(undefined, true);
                await collection.buildIndex();
        ...
        test(
            ...
            async () => {
                ...
                const collection = createTestWebsiteCollection(testWebsites, true);
                await collection.buildIndex();
                ...
                expect(knowledge).toBeDefined();
                if (knowledge) {
                    expect(knowledge.topics).toContain("OpenAI Research - Artificial Intelligence");
                    expect(knowledge.entities.some(e => e.name === "openai.com")).toBe(true);
                }
            },
            testTimeout
        );
        ...
        test(
            ...
            async () => {
                ...
                const collection = createTestWebsiteCollection(largeTestData, true);

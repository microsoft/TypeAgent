...
export class ActionExtractor {
    private readonly userAgent = 'Mozilla/5.0 (compatible; TypeAgent-ActionDetector/1.0)';
    ...
    createActionSummary(actions: DetectedAction[]): ActionSummary {
        ...
        const highConfidenceActions = actions.filter(a => a.confidence > 0.8).length;
        const primaryAction = actions.sort((a, b) => b.confidence - a.confidence)[0];

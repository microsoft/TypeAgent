...
export class Analytics {
     ...
     */
    recordFailure(mode: ExtractionMode, processingTime: number, error: Error): void {
        const record: ExtractionRecord = {
            timestamp: new Date(),
            mode,
            success: false,
            ...
            qualityMetrics: {
                ...
                knowledgeStrategy: EXTRACTION_MODE_CONFIGS[mode].knowledgeStrategy
            },
            error: error.message
        };
    ...
    getAnalyticsSummary(): ExtractionAnalytics {
        ...
        const modeUsage: Record<ExtractionMode, number> = {
            basic: 0, content: 0, actions: 0, full: 0
        };
        ...
        const modeAverageTime: Record<ExtractionMode, number> = {
            basic: modeUsage.basic > 0 ? modeTimeSum.basic / modeUsage.basic : 0,
            content: modeUsage.content > 0 ? modeTimeSum.content / modeUsage.content : 0,
            actions: modeUsage.actions > 0 ? modeTimeSum.actions / modeUsage.actions : 0,
            full: modeUsage.full > 0 ? modeTimeSum.full / modeUsage.full : 0
        };

        const modeSuccessRate: Record<ExtractionMode, number> = {
            basic: modeUsage.basic > 0 ? (modeSuccesses.basic / modeUsage.basic) * 100 : 0,
            content: modeUsage.content > 0 ? (modeSuccesses.content / modeUsage.content) * 100 : 0,
            actions: modeUsage.actions > 0 ? (modeSuccesses.actions / modeUsage.actions) * 100 : 0,
            full: modeUsage.full > 0 ? (modeSuccesses.full / modeUsage.full) * 100 : 0
        };

        // AI vs Basic analytics
        const aiRecords = this.records.filter(r => 
            EXTRACTION_MODE_CONFIGS[r.mode].usesAI
        );
        const basicRecords = this.records.filter(r => r.mode === 'basic');
        const aiSuccessful = aiRecords.filter(r => r.success);

        // Performance categorization
        const fastExtractions = this.records.filter(r => r.processingTime < 1000).length;
        const mediumExtractions = this.records.filter(r => 
            r.processingTime >= 1000 && r.processingTime <= 5000
        ).length;
        const slowExtractions = this.records.filter(r => r.processingTime > 5000).length;
        ...
        return {
            ...
            averageProcessingTime: this.records.reduce((sum, r) => sum + r.processingTime, 0) / this.records.length,
            averageConfidence: successfulRecords.length > 0 
                ? successfulRecords.reduce((sum, r) => sum + r.qualityMetrics.confidence, 0) / successfulRecords.length 
                : 0,
            ...
            averageEntityCount: successfulRecords.length > 0
                ? successfulRecords.reduce((sum, r) => sum + r.qualityMetrics.entityCount, 0) / successfulRecords.length
                : 0,
            averageTopicCount: successfulRecords.length > 0
                ? successfulRecords.reduce((sum, r) => sum + r.qualityMetrics.topicCount, 0) / successfulRecords.length
                : 0,
            averageActionCount: successfulRecords.length > 0
                ? successfulRecords.reduce((sum, r) => sum + r.qualityMetrics.actionCount, 0) / successfulRecords.length
                : 0,
            
            extractionsLast24h: this.records.filter(r => r.timestamp >= last24h).length,
            extractionsLastWeek: this.records.filter(r => r.timestamp >= lastWeek).length,
            extractionsLastMonth: this.records.filter(r => r.timestamp >= lastMonth).length,
            
            aiExtractions: aiRecords.length,
            basicExtractions: basicRecords.length,
            aiSuccessRate: aiRecords.length > 0 ? (aiSuccessful.length / aiRecords.length) * 100 : 0,
     ...
     */
    getTopErrors(limit: number = 10): Array<{error: string, count: number, percentage: number}> {
    ...
    getPerformanceTrends(bucketSize: 'hour' | 'day' | 'week' = 'hour'): Array<{
        ...
        const bucketMillis = bucketSize === 'hour' ? 60 * 60 * 1000 : 
                           bucketSize === 'day' ? 24 * 60 * 60 * 1000 :
                           7 * 24 * 60 * 60 * 1000;

        const buckets = new Map<number, ExtractionRecord[]>();
        
        this.records.forEach(record => {
            const bucketKey = Math.floor(record.timestamp.getTime() / bucketMillis) * bucketMillis;
        ...
        return Array.from(buckets.entries())
            .map(([timestamp, records]) => {
                ...
                return {
                    ...
                    averageTime: records.reduce((sum, r) => sum + r.processingTime, 0) / records.length,

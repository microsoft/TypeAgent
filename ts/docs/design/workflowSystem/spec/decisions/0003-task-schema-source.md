...
Status: **Open.** Spec currently follows option 1 (spec is source of truth).
...
spec, in the task registry, or both?
...
2. **Drift detection between spec and task.** Does the engine compare what
   the spec says about a task to what the task actually declares about
   itself, and if so, when (validation time vs. runtime) and in which
   direction (subtype vs. equality)?

The current spec answers (1) implicitly with "the spec" and (2) with
...
### Option 1 - Spec is the source of truth (current spec wording)

Every task node restates `inputSchema` and `outputSchema`. The validator
runs against the spec alone. The task implementation is fully opaque at
validation time. At runtime, `outputSchema` is checked against the task's
return value (§5.2).

- **Pros.**
  1. Spec is fully self-contained: validation needs no registry. This
     matters for portability, offline tooling, schema review, and the
     "spec is one document" property cited in §8.13 Alt B.
  2. **Specialization.** A spec author can declare a _narrower_ schema at
...
- **Cons.**
  1. Real duplication: the same shape is written in the spec and in the
  ...
  2. **Asymmetric, late drift detection.** Runtime `outputSchema`
     ...
     - a spec `inputSchema` that disagrees with what the task actually
       requires (the task throws on an undeclared field, blamed at
       runtime),
     - a spec `outputSchema` that is _stricter_ than the task's actual
       return when the task happens to return a value passing the
       stricter shape (the disagreement is invisible until the task
       returns a value that fails the spec but would have passed its own
       contract),
     - any disagreement at validation time, before the workflow runs.

### Option 1' - Spec is authoritative, validator also checks for drift

Identical to Option 1 in spec shape. Adds one validator step: when the
...
- **equality** (spec must exactly match task) - strictest, no
  specialization,
- **subtype-in-the-right-direction** - spec `inputSchema` must be a
  _subtype_ of the task's input (the spec promises at most what the task
  accepts) and the task's output must be a subtype of spec
  `outputSchema` (the task promises at least what the spec consumes).
...
validation), the spec still validates standalone exactly as in Option 1.
...
- **Pros.** Everything Option 1 gets, plus:
  1. Drift caught statically and symmetrically (input and output, not just
     output).
  2. Costs nothing at the spec layer: no schema change, no fields added
...
- **Cons.**
  1. Spec is no longer self-contained. Validation requires the task
     registry. Offline review tools, archival validation, and
     cross-deployment portability all weaken.
  2. No specialization: the spec cannot narrow the schema at a call site.
  3. Schema evolution in the implementation silently changes the spec's
     meaning. A task that adds an optional input field would let
     previously-invalid specs become valid; a task that tightens a type
     would silently invalidate working specs.
  4. Conflicts with the existing minimalism rule (§1.2): introduces an
     implicit dependency on a registry whose structure is not in the spec.

### Option 3 - Hybrid: task declares, spec may narrow
...
- **Cons.**
  1. The default path requires a registry, weakening the
     "spec is self-contained" property in practice even though the
...
| ---------------------------------- | -------- | ---------- | -------- | ------------------------ |
| Spec self-contained                | yes      | yes        | no       | partial                  |
...
### IR vs. authoring (preamble: "The spec is an IR, not an authoring format")
...
> Authoring sugar belongs in a DSL that compiles to the explicit spec."

Schemas describe the shape of every value that flows through the spec, so
they are part of the data-flow contract. Removing them from the spec to
...
- Option 1: aligned. Schemas live at the spec layer where the contract
...
- Option 3: aligned **only if** the canonical form has the schemas
  ...
  spec to any other tool. If the absent-schema form is itself the
...
> and every transformation by reading the spec."
...
- Option 1: aligned.
- Option 2: weakened. "Reading the spec" no longer suffices to
...
> "Someone reading the spec should be able to predict what the engine
...
depends on a convention not visible in the spec ("if `inputSchema` is
...
- Option 1 proves producer/consumer compatibility at every spec-internal
  reference, but defers the spec/task agreement to runtime (and only on
  the output side). The seam between the spec and the task carries an
  un-proven assumption into runtime.
- Option 1' closes that seam: the spec/task agreement is also proved
...
spec/task seam is at the IR/implementation boundary, which the
...
### Minimization rule (preamble, also spec §1.2)

> "The spec schema should use the fewest concepts necessary to satisfy
...
written to formalize, applied at the spec/task seam rather than at the
...
The IR is verbose by design and the "spec is one self-contained document"
...
Adopt **Option 1'** for v1, with the relationship between spec and task
...
Option 1 behavior - the spec still validates standalone, but the
spec/task seam is not statically checked and is handled by the
runtime output check in §5.2.

Update the spec to:

- §4.1 - add a validation pass (between the type-resolution pass and the
  type-compatibility pass) for spec/task drift, gated on registry
...
- §8.16 - state the chosen design as "task is the envelope, spec is
...
- spec-v1.md §4.2 (compatibility) - the subtype relation that option 3
  would reuse for the spec-narrows-registry check.
- spec-v1.md §5.2 (task execution) - the runtime validation point that
  enforces the spec's `outputSchema` against the implementation.
- spec-v1.md §8.13 (shared schemas) - the `types` block that already
  reduces duplication within a single spec; orthogonal to this decision.
- spec-v1.md §8.1 notes (post-v1) - the DSL layer that would handle the

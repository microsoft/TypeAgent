...
Design documentation for the workflow execution engine and its IR (workflow spec).
...
3. **[spec/spec-v1.md](spec/spec-v1.md)** - the v1 IR spec. The single authoritative description of the workflow document format, validation rules, and execution semantics.
4. **[spec/decisions/](spec/decisions/)** - per-decision records for the spec (one focused doc per significant choice).
5. **[spec/post-v1/](spec/post-v1/)** - sketches of spec features deferred past v1.
...
│   └── principle-gaps.md
├── spec/                              the IR contract (component: spec)
│   ├── spec-v1.md                     authoritative v1 spec
│   ├── decisions/                     numbered design records (0001-, 0002-, ...)
│   └── post-v1/                       deferred spec features
...
design work begins. Each will follow the same internal pattern as `spec/`:
...
- **Component** (top-level folders): which sub-system the doc is about. Today only the spec is deeply designed; other components are placeholders or empty.
- **Genre** (within each component): what kind of doc it is - an authoritative description (`spec-v1.md`, `plugin-api.md`, ...), a numbered decision record (`decisions/NNNN-*.md`), or a forward-looking sketch (`post-v1/*.md`).
...
Each doc under `spec/`, `engineering/`, and any future component folder carries a status header:
...
- **A new design choice for an existing component:** add a numbered file under `<component>/decisions/`. Use [spec/decisions/0001-bound-outputs.md](spec/decisions/0001-bound-outputs.md) as the template.

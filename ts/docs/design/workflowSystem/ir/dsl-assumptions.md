...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1  | Codegen can splice DSL-produced IR fragments without violating boundary closure ([§1.4](ir-v1.md)). Each scope stays closed; the DSL never needs values to escape upward.         | §1.4 boundary closure, the loop-body-as-sub-IR shape, and the absence of a `block` scope or export mechanism in v1.                  | Least-validated. If a DSL needs to hoist names or values across scopes, see [post-v1/block-scope.md](post-v1/block-scope.md).                                                                                    |
| S2  | The DSL stays within the four v1 node kinds (`task`, `branch`, `loop`, `handler`). Richer source surfaces (`try/catch`, parallel blocks, sub-procedures) are lowered or rejected. | [§1.3](ir-v1.md) minimalism (exactly four kinds) and the deferral of sub-workflows / block scope / nested loops to [§2.2](ir-v1.md). | If the DSL repeatedly needs a fifth kind, the deferred features in §2.2 become v1 pressure rather than v2 work. See [revisit-triggers.md](revisit-triggers.md) row 7 (codegen coverage).                         |
| S3  | The DSL preserves the constants-as-global model ([§8.9](ir-v1.md)) rather than introducing per-scope constants whose names then collide at the IR level.                          | §8.9 (constants are workflow-global) and the scope-closure pass that excludes constants from the closure restriction.                | If the DSL surface has block-scoped `const` declarations, the DSL must alpha-rename to a single workflow-global namespace before lowering. If that gets painful, scope-local constants is an additive IR change. |
...
| ID  | Assumption                                                                                                                                                                                                                                                                                                                                                                                                                  | What in v1 depends on it                                                                                                                        | How to check / when to revisit                                                                                                                                                                            |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | The DSL fills required IR fields the author may omit, with sensible defaults or inferences. This includes loop `maxIterations` ([§3.7](ir-v1.md)), branch `default` ([§3.6](ir-v1.md), [§8.3](ir-v1.md)), `outputBinding` on every value-producing scope ([§3.1](ir-v1.md), [§3.7](ir-v1.md), [§8.10](ir-v1.md)), loop boundary `inputs` ([§3.7](ir-v1.md)), and node `stateWrites` ([§3.7.1](ir-v1.md), [§8.5](ir-v1.md)). | Every IR field that is required-no-default. The IR's "no surprise defaults" stance (P5) means the engine never invents these; the DSL must.     | If a future DSL surface forces authors to write all of these explicitly and that becomes the dominant author complaint, several "required, no default" choices become candidates for IR-level relaxation. |
| A2  | The DSL surfaces ergonomic equivalents for IR shapes that are structurally explicit but verbose: discriminant switch as `match` ([§8.3](ir-v1.md)), loop sentinels as `continue` / `break` ([§8.4](ir-v1.md)), `stateWrites` as assignment syntax ([§8.5](ir-v1.md)), object references as dotted names ([§8.2](ir-v1.md)), and named node IDs from source position ([§3.2](ir-v1.md)).                                     | The "no sugar in the IR" stance ([§1.2](ir-v1.md)). If the DSL also has no sugar for these, the LLM and humans see the IR's verbosity directly. | If the DSL ships without these surfaces and authors complain, the answer is normally to extend the DSL, not the IR. The IR-side decisions only revisit if no DSL surface can be made to work.             |
| A3  | The bind-switch hide-by-default ([§8.15](ir-v1.md)) ergonomics are remediable at the DSL surface (DSL picks whatever the user-facing default should be; codegen wraps).                                                                                                                                                                                                                                                     | [decision 0001](decisions/0001-bound-outputs.md) and §8.15. Engine wants hide-by-default for liveness; users may want publish-by-default.       | If the DSL surface ends up _always_ publishing and the IR's hide-by-default never matches what humans authored, the IR default is engine-only and that is acceptable.                                     |

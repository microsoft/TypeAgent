...
### 2026-06-27 — Host contributes the typed `@source add` command handlers; core stays taxonomy-free

- **Milestone / item:** Layering follow-up (post-M4)
- **Type:** Layering correction
- **Design ref:** §4.1, §5, §10 (`kind` seam)
- **Decision:** The dispatcher core no longer knows the source-kind taxonomy or the
  `@source add` grammar. Removed `InstallSourceKind` and the `Path`/`Feed`/`Catalog`/union
  `InstallSourceConfig` types from core; `InstallSource.kind` is now an opaque `string`.
  Core exposes a single rendering shape `InstallSourceInfo { name, kind, detail }` (consumed
  by `@source list`), and the core `@source` table contributes only `list`/`order`/`remove`.
  The **host contributes the `@source add` subcommands directly**: `AppAgentInstaller` gains
  `sourceCommands?(): CommandHandlerTable`, and the system agent merges that table into the
  `@source add` node dynamically (`getSystemHandlers(systemContext)` in `systemAgent.ts`,
  also used by `@help`). The host (`default-agent-provider`) owns the moved taxonomy
  (`installSources/config.ts`) and authors the typed `feed`/`catalog`/`path` handlers
  (`installSources/addSource.ts → getAddSourceCommandHandlers(registry)`) — each a real
  `CommandHandler` with typed args + flags and per-kind validation that calls
  `registry.add(config)`. A host-local `DefaultInstallSourceRegistry extends
  InstallSourceRegistry` keeps the typed `add(config)` for these handlers, seeding, and tests.
- **Rationale:** "How sources are added" is a host concern — a future host may need to run
  auth UI (e.g. feed credentials) during add. Keeping the kind taxonomy and npm/Azure
  vocabulary out of core makes the core provider-agnostic and lets new source kinds ship
  entirely in the host. Command **resolution and execution already flow through
  `appAgent.getCommands(sessionContext)`**, whose `sessionContext.agentContext` is the
  system agent's `CommandHandlerContext` (carrying `agentInstaller`). So making
  `systemAgent.getCommands/executeCommand/getCommandCompletion` context-aware is enough to
  merge host-contributed subcommands for parsing, completion, execution, and help. This
  supersedes the earlier belief that the static `systemHandlers` constant could not receive
  the per-dispatcher installer — host-contributed *typed* subcommands are in fact feasible.
- **Tradeoff:** `@source add feed/catalog/path` now get first-class typed flag help and
  completion (same UX as built-in commands), with the grammar, descriptors, and validation
  all in the host. The seam is optional (absent → `@source add` unavailable, like
  `update`/`sources`). Validation coverage lives in the host test
  (`installSourcesAddSource.spec.ts`, exercising the handlers directly); the core handler
  test (`installCommandHandlers.spec.ts`) no longer references `add`.
- **Supersedes:** the two earlier same-day explorations of this seam — the opaque
  `addSource(spec: string)` + `implicitQuotes` handler, and the `rest` arg type with
  `addSource(args: string[])`. Both are reverted: the core `addSource` seam and the `rest`
  argument kind (`ArgDefinition.rest`) were removed. The `rest` arg type may be re-added
  later if a handler genuinely needs a verbatim token tail.
- **Design updated?** no (interface-level layering refinement; record/JSON shapes unchanged)

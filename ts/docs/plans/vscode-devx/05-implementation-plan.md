...
- **Channel protocol** (served by the `studio` agent, types in `@typeagent/core` / the agent): `agent-rpc` `createRpc` over the WS — typed `invoke` (`StudioRequest`/`StudioResponse<T>`) + a `StudioEvent` subscription (`healthChanged`, `replayRow`, `replayCompleted`, `traceAppended`) with subscription ids, schema version, cancellation, and paging. This WS protocol is the **canonical typed Studio API**; `studio` actions + MCP tools wrap the same runtime methods. See [`STUDIO-AGENT.md` §8](./STUDIO-AGENT.md).

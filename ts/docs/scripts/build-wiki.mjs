 ...
 *   node ts/docs/scripts/build-wiki.mjs --check    # CI: fail on uncategorized arch docs
...
const ARCH_GROUPS = [
    ...
    },
    {
        ...
        files: [
            ...
            "contextSelector-report.md",
            "onboarding-keyword-generation-design.md",
        ],
    },
    {
        dir: "memory",
    ...
    },
    {
        dir: "telemetry",
        name: "Telemetry",
        files: ["opentelemetry.md", "local-telemetry-debugging.md"],
    },
    {
        dir: "workflows",
...
async function findUnlistedArchitectureDocs() {
    const unlisted = [];
    const archDir = path.join(docsRoot, "architecture");
    for (const group of ARCH_GROUPS) {
        if (group.designTree) continue;
        const listed = new Set(group.files.map((file) => file.toLowerCase()));
        let entries = [];
        try {
            entries = await fs.readdir(path.join(archDir, group.dir), {
                withFileTypes: true,
            });
        } catch {
            continue;
        ...
        for (const entry of entries) {
            if (
                ...
                !listed.has(entry.name.toLowerCase())
            ) {
                unlisted.push(`${group.dir}/${entry.name}`);
            }
        }
    }
...
async function main() {
    if (checkOnly) {
        ...
        const unlisted = await findUnlistedArchitectureDocs();
        const uncategorized = [
            ...(unc?.items.map((item) => item.href) ?? []),
            ...unlisted,
        ];
        if (uncategorized.length > 0) {
            console.error(
                `Uncategorized architecture docs: ${uncategorized.join(", ")}\n` +
